...
class Merger(object):
    ...
    def mergeLists(self, out, lst):
        if not allEqualTo(out, lst, len):
            raise LengthsDiffer(
                self, expected=len(out), got=[len(x) for x in lst]
            )
...
def _merge_GlyphOrders(font, lst, values_lst=None, default=None):
    ...
    if values_lst is None:
        ...
        dict_sets = [
            {g: v for g, v in zip(l, vs)} for l, vs in zip(lst, values_lst)
        ]
    combined = set()
    ...
    if values_lst is None:
        ...
        padded = [
            [
                dict_set[glyph] if glyph in dict_set else default
                for glyph in order
            ]
            for dict_set in dict_sets
        ]
    ...
    for self in subtables:
        ...
        if self.Format == 1:
            ...
            raise UnsupportedFormat(
                merger, subtable="single positioning lookup"
            )
    return None
...
def merge(merger, self, lst):
    self.ValueFormat = valueFormat = reduce(
        int.__or__, [l.ValueFormat for l in lst], 0
    )
    ...
    if all(v.Format == 1 for v in lst) and all(
        ...
        if valueFormat != 0:
            ...
            merger.mergeThings(
                self.Value,
                [
                    v.Value if v.Value is not None else otBase.ValueRecord()
                    for v in lst
                ],
            )
...
def merge(merger, self, lst):
    ...
    for glyph in glyphs:
        ...
        pvr.Value1 = (
            otBase.ValueRecord(merger.valueFormat1)
            if merger.valueFormat1
            else None
        )
        pvr.Value2 = (
            otBase.ValueRecord(merger.valueFormat2)
...
def _PairPosFormat2_merge(self, lst, merger):
    ...
    # Align coverages
    glyphs, _ = _merge_GlyphOrders(
        merger.font, [v.Coverage.glyphs for v in lst]
    )
    self.Coverage.glyphs = glyphs
...
def _MarkBasePosFormat1_merge(self, lst, merger, Mark="Mark", Base="Base"):
    ...
    for g, glyphRecords in zip(MarkCoverageGlyphs, zip(*MarkRecords)):
        ...
        if not allEqual(allClasses):
            raise ShouldBeConstant(
                merger, expected=allClasses[0], got=allClasses
            )
        else:
            rec = ot.MarkRecord()
    ...
    for g, glyphRecords in zip(BaseCoverageGlyphs, zip(*BaseRecords)):
        if allNone(glyphRecords):
            ...
            glyphAnchors = [
                [] if r is None else getattr(r, Base + "Anchor")
...
def merge(merger, self, lst):
    ...
    if self.Format == 1:
        ...
        raise UnsupportedFormat(
            merger, subtable="mark-to-base positioning lookup"
        )
...
def merge(merger, self, lst):
    ...
    if self.Format == 1:
        ...
        raise UnsupportedFormat(
            merger, subtable="mark-to-mark positioning lookup"
        )
...
def _Lookup_PairPosFormat2_subtables_flatten(lst, font):
    ...
    matrices = _PairPosFormat2_align_matrices(
        self, lst, font, transparent=True
    )
...
def _Lookup_SinglePos_subtables_flatten(lst, font, min_inclusive_rec_format):
    glyphs, _ = _merge_GlyphOrders(
        font, [v.Coverage.glyphs for v in lst], None
    )
...
def merge(merger, self, lst):
    ...
    if isPairPos:
        ...
        isSinglePos = self.SubTable and isinstance(
            self.SubTable[0], ot.SinglePos
        )
        if isSinglePos:
...
def merge(merger, self, lst):
    ...
    masters = [
        {rec.BaseGlyph: rec for rec in m.BaseGlyphPaintRecord} for m in lst
    ]
...
def merge(merger, self, lst):
    ...
    ), "BaseGlyphList must be merged before LayerList"
...
def _merge_PaintColrLayers(self, out, lst):
    ...
    if (
        len(out_layers) == 1
        and out_layers[0].Format == ot.PaintFormat.PaintColrLayers
    ):
...
def merge(merger, self, lst):
    ...
    staticAttrs = (
        c.name for c in self.getConverters() if c.name not in varAttrs
    )
