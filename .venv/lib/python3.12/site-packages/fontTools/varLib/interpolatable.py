    ...
    for glyph_name in glyphs:
        ...
        for m1idx in order:
            ...
            for ix, (contour0, contour1) in enumerate(
                ...
                if contour and (m0Vectors[ix][0] < 0) == (
                    m1Vectors[ix][0] < 0
                ):
                    ...
                    for overweight, problem_type in enumerate(
                        ...
                        if (
                            ...
                            overweight
                            and 1e-5 + expectedSize / tolerance < midSize
                        ):
                            try:
                                if overweight:
            ...
            # If contour-order is wrong, adjust it
            if (
                matchings[m1idx] is not None and m1
            ):  # m1 is empty for composite glyphs
            ...
            for ix, (contour0, contour1) in enumerate(zip(m0, m1)):
                ...
                for i in range(len(contour0)):
                    ...
                    sin0 = (
                        d0_prev.real * d0_next.imag
                        - d0_prev.imag * d0_next.real
                    )
                    sin1 = (
                        d1_prev.real * d1_next.imag
                        - d1_prev.imag * d1_next.real
                    )
                    try:
                    ...
                    dot0 = (
                        d0_prev.real * d0_next.real
                        + d0_prev.imag * d0_next.imag
                    )
                    dot1 = (
                        d1_prev.real * d1_next.real
                        + d1_prev.imag * d1_next.imag
                    )
                    if dot0 < 0 or dot1 < 0:
                        # Sharp corner.
                        continue
                    ...
                    sin_mid = (
                        mid_d0.real * mid_d1.imag - mid_d0.imag * mid_d1.real
                    )
                    try:
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        action="store",
        type=float,
        help="Error tolerance. Between 0 and 1. Default %s"
        % DEFAULT_TOLERANCE,
    )
    parser.add_argument(
        "--kinkiness",
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Run verbosely."
    )
    parser.add_argument(
    ...
    if len(args.inputs) == 1:
        ...
        if args.inputs[0].endswith(".designspace"):
            ...
            axis_triples = {
                k: tuple(
                    piecewiseLinearMap(v, dict(axis_mappings[k])) for v in vv
                )
        ...
        elif args.inputs[0].endswith(".ttf") or args.inputs[0].endswith(
            ".otf"
        ):
            ...
            if "avar" in font:
                ...
                if getattr(avar.table, "VarStore", None):
                    axisMapping = {
                        tag: {-1: -1, 0: 0, 1: 1} for tag in axisMapping
                    }
                    normalized = True
                else:
                ...
                for glyphname in glyphs:
                    ...
                    for vsindex in vsindices:
                        ...
                        for regionIndex in varData.VarRegionIndex:
                            ...
                            glyphset[glyphname] = ttGlyphSets[locTuple][
                                glyphname
                            ]
            ...
            for locTuple in sorted(
                glyphsets.keys(), key=lambda v: (len(v), v)
            ):
                name = (
                    "'"
                    + " ".join(
    ...
    if len(fonts) < 2:
        log.warning(
            "Font file does not seem to be variable. Nothing to check."
        )
        return
    ...
    if not glyphs:
        glyphs = sorted(
            set([gn for glyphset in glyphsets for gn in glyphset.keys()])
        )
    ...
    kinkiness = (
        args.kinkiness if args.kinkiness is not None else DEFAULT_KINKINESS
    )
    ...
    try:
            ...
            if args.json:
                ...
                for glyphname, p in problems_gen:
                    ...
                    if p["type"] == InterpolatableProblem.MISSING:
                        ...
                        p["type"] == InterpolatableProblem.NODE_INCOMPATIBILITY
                    ):
                        print(
        ...
        if args.html:
            ...
            svgs = []
            glyph_starts = {}
            with InterpolatableSVG(
                svgs, glyphsets=glyphsets, names=names
            ) as svg:
                svg.add_title_page(
                    original_args_inputs,
            ...
            with open(ensure_parent_dir(args.html), "wb") as f:
                ...
                f.write(
                    b"<title>fonttools varLib.interpolatable report</title>\n"
                )
                for i, svg in enumerate(svgs):
                    if i in glyph_starts:
                        f.write(
                            f"<h1>Glyph {glyph_starts[i]}</h1>\n".encode(
                                "utf-8"
                            )
                        )
                    f.write(
                        "<img src='data:image/svg+xml;base64,".encode("utf-8")
                    )
