...
class NormalizedAxisTriple(AxisTriple):
    ...
    def __post_init__(self):
        if self.default is None:
            object.__setattr__(
                self, "default", max(self.minimum, min(self.maximum, 0))
            )
...
class NormalizedAxisTripleAndDistances(AxisTriple):
    ...
    def renormalizeValue(self, v, extrapolate=True):
        ...
        if default < 0:
            return -self.reverse_negate().renormalizeValue(
...
class _BaseAxisLimits(Mapping[str, AxisTriple]):
    ...
    def pinnedLocation(self) -> Dict[str, float]:
        ...
        return {
            k: v.default for k, v in self.items() if v.minimum == v.maximum
        }
...
class AxisLimits(_BaseAxisLimits):
        ...
        for k, v in dict(*args, **kwargs).items():
            if v is None:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid axis limits for {k!r}: {v!r}"
                    ) from e
                data[k] = triple
    ...
    def limitAxesAndPopulateDefaults(self, varfont) -> "AxisLimits":
        ...
        for axisTag, triple in self.items():
            ...
            if triple is None:
                ...
                newLimits[axisTag] = triple.limitRangeAndPopulateDefaults(
                    fvarTriple
                )
    ...
    def normalize(self, varfont, usingAvar=True) -> "NormalizedAxisLimits":
        ...
        for axis_tag, triple in axes.items():
            ...
            normalizedLimits[axis_tag] = NormalizedAxisTripleAndDistances(
                *(
                    normalize(v, triple, avarMapping)
                    for v in (minV, defaultV, maxV)
                ),
                distanceNegative,
                distancePositive,
            )
...
def instantiateVARC(varfont, axisLimits):
    ...
    axisMap = [
        i for i, axis in enumerate(fvarAxes) if axis.axisTag not in location
    ]
    ...
    for axisTag, axisLimit in sorted(axisLimits.items()):
        ...
        for var in variations:
            newVariations.extend(
                changeTupleVariationAxisLimit(var, axisTag, axisLimit)
            )
...
def changeTupleVariationAxisLimit(var, axisTag, axisLimit):
    ...
    for scalar, tent in solutions:
        newVar = (
            TupleVariation(var.axes, var.coordinates)
            if len(solutions) > 1
            else var
        )
        if tent is None:
...
def instantiateCFF2(
    ...
    *,
    ...
    def getNumRegions(vsindex):
        return varStore.VarData[
            vsindex if vsindex is not None else 0
        ].VarRegionCount
    ...
    for cs in charStrings:
        ...
        if specialize:
            commands = specializeCommands(
                commands, generalizeFirst=not generalize
            )
    ...
    for opcode, name, arg_type, default, converter in privateDictOperators2:
        ...
        for private in privateDicts:
            ...
            for value in values:
                ...
                newValues.extend(
                    v[:-1] if isinstance(v, list) else v for v in value
                )
    ...
    for commands in allCommands:
        if any(
            isinstance(arg, list) for command in commands for arg in command[1]
        ):
            vsindex = 0
            for command in commands:
                if command[0] == "vsindex":
            ...
            commands[:] = [
                command for command in commands if command[0] != "vsindex"
            ]
...
def _instantiateGvarGlyph(
    ...
    coordinates, ctrl = glyf._getCoordinatesAndControls(
...
def verticalMetricsKeptInSync(varfont):
    ...
    metrics_are_synced = current_os2_vmetrics == [
        getattr(varfont["hhea"], attr)
        for attr in ("ascender", "descender", "lineGap")
    ]
...
def _remapVarIdxMap(table, attrName, varIndexMapping, glyphOrder):
    ...
    newMapping = [
        varIndexMapping[oldMapping[glyphName]] for glyphName in glyphOrder
    ]
    ...
    if "glyf" not in varfont:
        ...
        if metricsTag in varfont:
            ...
            for glyphName, (advanceWidth, sb) in metrics.items():
                ...
                metrics[glyphName] = (
                    advanceWidth + round(defaultDeltas[varIdx]),
                    sb,
                )
    ...
    def asItemVarStore(self):
        ...
        for variations, itemCount in zip(self.tupleVarData, self.itemCounts):
            if variations:
                ...
                varRegionIndices = [
                    regionOrder.index(frozenset(var.axes.items()))
                    for var in variations
                ]
                varDataItems = list(
...
def instantiateOTL(varfont, axisLimits):
    ...
    merger = MutatorMerger(
        varfont,
        defaultDeltas,
        deleteVariations=(not varStore.VarRegionList.Region),
    )
    ...
    for axisTag, mapping in segments.items():
        ...
        if mapping and axisTag in normalizedRanges:
            ...
            for fromCoord, toCoord in mapping.items():
                if (
                    fromCoord < axisRange.minimum
                    or fromCoord > axisRange.maximum
                ):
                    continue
...
def instantiateFvar(varfont, axisLimits):
    ...
    for instance in fvar.instances:
        if any(
            instance.coordinates[axis] != value
...
def axisValuesFromAxisLimits(stat, axisLimits):
    ...
    for axisValueTable in stat.AxisValueArray.AxisValue:
        ...
        if axisValueFormat in (1, 2, 3):
            ...
            log.warning(
                "Unknown AxisValue table format (%s); ignored", axisValueFormat
            )
    ...
    if "fvar" not in varfont:
        if "glyf" in varfont:
            if overlap == OverlapMode.KEEP_AND_SET_FLAGS:
                ...
                OverlapMode.REMOVE,
                OverlapMode.REMOVE_AND_IGNORE_ERRORS,
            ):
...
def setRibbiBits(font):
    ...
    english_ribbi_style = font["name"].getName(
        names.NameID.SUBFAMILY_NAME, 3, 1, 0x409
    )
...
def parseArgs(args):
    ...
    configLogger(
        level=(
            "DEBUG"
            if options.verbose
            else "ERROR" if options.quiet else "INFO"
        )
    )
