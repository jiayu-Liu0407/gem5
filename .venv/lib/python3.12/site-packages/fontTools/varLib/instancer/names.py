...
def getVariationNameIDs(varfont):
    ...
    if "STAT" in varfont:
        ...
        for axis in (
            stat.DesignAxisRecord.Axis if stat.DesignAxisRecord else ()
        ):
            used.append(axis.AxisNameID)
        for value in (
            stat.AxisValueArray.AxisValue if stat.AxisValueArray else ()
        ):
...
def pruningUnusedNames(varfont):
    ...
    varfont["name"].names[:] = [
        ...
        if record.nameID not in exclude
    ]
    if "ltag" in varfont:
...
def updateNameTable(varfont, axisLimits):
    ...
    if "STAT" not in varfont:
        raise ValueError(
            "Cannot update name table since there is no STAT table."
        )
    stat = varfont["STAT"].table
    if not stat.AxisValueArray:
        raise ValueError(
            "Cannot update name table since there are no STAT Axis Values"
        )
    fvar = varfont["fvar"]
    ...
    checkAxisValuesExist(
        stat, axisValueTables, defaultAxisCoords.pinnedLocation()
    )
    ...
    for platform in platforms:
        ...
        if not typoSubFamilyName and not subFamilyName:
            if elidedNameIsRibbi:
                ...
                typoSubFamilyName = getName(
                    elidedNameID, *platform
                ).toUnicode()
...
def _updateNameTableStyleRecords(
    ...
    if not all([currentFamilyName, currentStyleName]):
        raise ValueError(
            f"Missing required NameIDs 1 and 2 for platform {platform}"
        )
    ...
    newFamilyName = (
        nameIDs.get(NameID.TYPOGRAPHIC_FAMILY_NAME)
        or nameIDs[NameID.FAMILY_NAME]
    )
    newStyleName = (
