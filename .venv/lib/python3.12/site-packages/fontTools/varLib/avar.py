            ...
            for v in (minV, maxV):
                ...
                for pin in pins.keys():
                    ...
                    for candidateIdx in range(i - 1, -1, -1):
                        ...
                        candidate = {
                            k: defaultV
                            for k, (_, defaultV, _) in candidate.items()
                        }
...
def mappings_from_avar(font, denormalize=True):
    ...
    if getattr(avar, "majorVersion", 1) == 2:
        ...
        inputLocations = [
            ...
            for t in sorted(
                ...
                key=lambda t: (
                    len(t),
                    tuple(axisIndexes[tag] for tag, _ in t),
                ),
            )
        ]
    ...
    if denormalize:
        ...
        for i, (inputLoc, outputLoc) in enumerate(mappings):
            inputLoc = {
                tag: _denormalize(val, axisMap[tag])
                for tag, val in inputLoc.items()
            }
            outputLoc = {
                tag: _denormalize(val, axisMap[tag])
                for tag, val in outputLoc.items()
            }
