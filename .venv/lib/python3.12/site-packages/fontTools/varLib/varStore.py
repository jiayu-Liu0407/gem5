...
class VarStoreInstancer(object):
    ...
    def interpolateFromDeltas(self, varDataIndex, deltas):
        varData = self._varData
        scalars = [
            self._getScalar(ri) for ri in varData[varDataIndex].VarRegionIndex
        ]
        return self.interpolateFromDeltasAndScalars(deltas, scalars)
    ...
    for major, data in enumerate(varData):
        ...
        if major == 0 and retainFirstMap:
            for minor in range(len(items)):
                newItems.append(
                    items[minor]
                    if minor in usedMinors
                    else [0] * len(items[minor])
                )
...
def VarStore_prune_regions(
    self, *, VarData="VarData", VarRegionList="VarRegionList"
):
...
class _Encoding(object):
    ...
    def gain_from_merging(self, other_encoding):
        ...
        combined_overhead = _Encoding._characteristic_overhead(
            combined_columns
        )
        combined_gain = (
            +self.overhead
            ...
            - (combined_width - other_encoding.width)
            * len(other_encoding.items)
        )
...
def VarStore_optimize(self, use_NO_VARIATION_INDEX=True, quantization=1):
    ...
    for k, v in front_mapping.items():
        varidx_map[k] = (
            back_mapping[v] if v is not None else NO_VARIATION_INDEX
        )
