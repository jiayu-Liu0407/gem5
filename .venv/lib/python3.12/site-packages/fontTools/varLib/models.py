...
def normalizeValue(v, triple, extrapolate=False):
    ...
    if (v < default and lower != default) or (
        v > default and upper == default
    ):
...
def normalizeLocation(location, axes, extrapolate=False, *, validate=False):
    ...
    """
    if validate:
        assert set(location.keys()) <= set(axes.keys()), set(
            location.keys()
        ) - set(axes.keys())
    out = {}
...
def supportScalar(
    location, support, ot=True, extrapolate=False, axisRanges=None
):
...
class VariationModel(object):
    ...
    def __init__(
        ...
        if len(set(tuple(sorted(l.items())) for l in locations)) != len(
            locations
        ):
        ...
        locations = [
            {k: v for k, v in loc.items() if v != 0.0} for loc in locations
        ]
    ...
    def getSubModel(self, items):
        ...
        if subModel is None:
            subModel = VariationModel(
                subList(key, self.origLocations), self.axisOrder
            )
        ...
        def getKey(axisPoints, axisOrder):
            ...
            def key(loc):
                ...
                orderedAxes.extend(
                    [
                        axis
                        for axis in sorted(loc.keys())
                        if axis not in axisOrder
                    ]
                )
                return (
    ...
    def reorderMasters(self, master_list, mapping):
        ...
        locations = [
            {k: v for k, v in loc.items() if v != 0.0}
    ...
    def interpolateFromMastersAndScalars(
        self, masterValues, scalars, *, round=noRound
    ):
