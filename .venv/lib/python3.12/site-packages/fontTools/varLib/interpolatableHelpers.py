...
try:
    ...
    min_cost_perfect_bipartite_matching = (
        min_cost_perfect_bipartite_matching_scipy
    )
except ImportError:
    try:
...
def add_isomorphisms(points, isomorphisms, reverse):
    ...
    for i in range(n):
        ...
        if b == reference_bits:
            isomorphisms.append(
                (
                    rot_list(vector, -i * mult),
                    n - 1 - i if reverse else i,
                    reverse,
                )
            )
    ...
    if locations:
        ...
        try:
            ...
            for i, j in itertools.combinations(range(len(locations)), 2):
                i_discrete_location = {
                    k: v
                    for k, v in zip(axes, vectors[i])
                    if k in discrete_axes
                }
                j_discrete_location = {
                    k: v
                    for k, v in zip(axes, vectors[j])
                    if k in discrete_axes
                }
