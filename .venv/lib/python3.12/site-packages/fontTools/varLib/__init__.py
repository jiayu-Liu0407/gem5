...
def _add_fvar(font, axes, instances: List[InstanceDescriptor]):
    ...
    macNames = any(
        nr.platformID == 1 for nr in getattr(nameTable, "names", ())
    )
    ...
    default_coordinates = {
        axis.axisTag: axis.defaultValue for axis in fvar.axes
    }
    ...
    for instance in instances:
        ...
        if (
            subfamilyNameID in {2, 17}
            and inst.coordinates == default_coordinates
        ):
        ...
        if psname is not None:
            psname = tostr(psname)
            inst.postscriptNameID = nameTable.addName(
                psname, platforms=platforms
            )
...
def _add_avar(font, axes, mappings, axisTags):
    ...
    if mappings:
        ...
        if not any(
            all(v == 0 for k, v in loc.items()) for loc in inputLocations
        ):
...
def _add_VHVAR(font, axisTags, tableFields, getAdvanceMetrics):
    ...
    if singleModel:
        ...
        for glyphName in glyphOrder:
            varData.addItem(
                vhAdvanceDeltasAndSupports[glyphName][0], round=noRound
            )
        varData.optimize()
    ...
    if vOrigDeltasAndSupports:
        ...
        for glyphName in glyphOrder:
            ...
            storeBuilder.setSupports(supports)
            vOrigMap[glyphName] = storeBuilder.storeDeltas(
                deltas, round=noRound
            )
...
def _get_advance_metrics(
    font, masterModel, master_ttfs, axisTags, tableFields
):
    tableTag = tableFields.tableTag
    glyphOrder = font.getGlyphOrder()
...
def _add_MVAR(font, masterModel, master_ttfs, axisTags):
    ...
    for tag, (tableTag, itemName) in sorted(
        MVAR_ENTRIES.items(), key=lambda kv: kv[1]
    ):
            ...
            if tableTag in font:
                ...
                for master in master_ttfs:
                    if tableTag not in master or (
                        tag in specialTags
                        and getattr(master[tableTag], itemName)
                        == specialTags[tag]
                    ):
                        tables.append(None)
                    else:
...
def _add_GSUB_feature_variations(
    ...
    def normalize(name, value):
        return models.normalizeLocation({name: value}, internal_axis_supports)[
            name
        ]
...
def _add_CFF2(varFont, model, master_fonts):
    ...
    ordered_fonts_list = model.reorderMasters(
        master_fonts, model.reverseMapping
    )
...
def set_default_weight_width_slant(font, location):
    if "OS/2" in font:
        ...
        if "wdth" in location:
            ...
            widthClass = otRound(
                models.piecewiseLinearMap(
                    widthValue, WDTH_VALUE_TO_OS2_WIDTH_CLASS
                )
            )
...
def drop_implied_oncurve_points(*masters: TTFont) -> int:
    ...
    for glyphName, glyphs in glyph_masters.items():
        try:
            ...
            log.warning(
                "Failed to drop implied oncurves for %r: %s", glyphName, e
            )
        else:
            count += len(dropped)
    return count
...
def build_many(
    ...
    doBuildStatFromDSv5 = (
        ...
        and (
            any(
                a.axisLabels or a.axisOrdering is not None
...
def build(
    ...
    set_default_weight_width_slant(
        vf,
        location={axis.axisTag: axis.defaultValue for axis in vf["fvar"].axes},
    )
...
def addGSUBFeatureVariations(
    vf, designspace, featureTags=(), *, log_enabled=False
):
...
def main(args=None):
    ...
    if not vf_descriptors:
        parser.error(
            f"No variable fonts in given designspace {designspace.path!r}"
        )
    ...
    if len(vfs_to_build) == 1 and options.outfile is not None:
        ...
        for vf in vfs_to_build:
            filename = (
                vf.filename if vf.filename is not None else vf.name + ".{ext}"
            )
            vf_name_to_output_path[vf.name] = os.path.join(
                output_dir, filename
            )
