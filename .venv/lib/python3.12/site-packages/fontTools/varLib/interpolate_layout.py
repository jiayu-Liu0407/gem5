...
def interpolate_layout(
    designspace, loc, master_finder=lambda s: s, mapped=False
):
    """
...
def main(args=None):
    ...
    if not args.output:
        args.output = (
            os.path.splitext(args.designspace_filename)[0] + "-instance.ttf"
        )

    configLogger(level=args.loglevel)

    finder = lambda s: s.replace(
        "master_ufo", "master_ttf_interpolatable"
    ).replace(".ufo", ".ttf")
