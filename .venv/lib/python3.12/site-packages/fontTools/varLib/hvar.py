...
def _get_advance_metrics(font, axisTags, tableFields):
    ...
    for glyphName in glyphOrder:
        ...
        for tv in variations:
            ...
            phantoms[0] = (
                phantoms[0][phantomIndex] if phantoms[0] is not None else 0
            )
            phantoms[1] = (
                phantoms[1][phantomIndex] if phantoms[1] is not None else 0
            )
...
def add_HVAR(font):
    ...
    getAdvanceMetrics = partial(
        _get_advance_metrics, font, axisTags, HVAR_FIELDS
    )
...
def main(args=None):
    ...
    if options.output_file is None:
        outfile = makeOutputFileName(
            options.font, overWrite=True, suffix=".hvar"
        )
    else:
