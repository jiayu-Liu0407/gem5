...
def iup_delta(
    ...
    assert (
        sorted(ends) == ends
        and len(coords) == (ends[-1] + 1 if ends else 0) + 4
    )
    n = len(coords)
...
def can_iup_in_between(
    ...
    assert j - i >= 2
    interp = iup_segment(
        coords[i + 1 : j], coords[i], deltas[i], coords[j], deltas[j]
    )
    deltas = deltas[i + 1 : j]
    ...
    for i in range(len(deltas) - 1, -1, -1):
        ...
        for j in (0, 1):  # For X and for Y
            ...
            # Mark it forced.
                ...
                if not (
                    min(d1, d2) - tolerance <= dj <= max(d1, d2) + tolerance
                ):
                    force = True
...
def _iup_contour_optimize_dp(
    ...
    for i in range(0, n):
        ...
        for j in range(i - 2, max(i - lookback, -2), -1):
            ...
            if cost < best_cost and can_iup_in_between(
                deltas, coords, j, i, tolerance
            ):
...
def iup_contour_optimize(
    ...
    if forced:
        ...
        chain, costs = _iup_contour_optimize_dp(
            deltas, coords, forced, tolerance
        )
