...
def sanitizeWeight(userTriple, designTriple, pins, measurements):
    ...
    if abs(calculatedDefaultVal - userTriple[1]) / userTriple[1] > 0.05:
        log.warning(
            "Calculated weight axis default does not match user input."
        )
...
def sanitizeSlant(userTriple, designTriple, pins, measurements):
    ...
    if (
        ...
        log.warning(
            "Calculated slant axis min/default/max do not match user input."
        )
        log.warning(
...
def planAxis(
    ...
    log.info(
        "Axis limits min %g / default %g / max %g",
        minValue,
        defaultValue,
        maxValue,
    )
    ...
    if designLimits is not None:
        log.info(
            "Axis design-limits min %g / default %g / max %g", *designLimits
        )
    else:
        designLimits = triple
    ...
    for (rangeMin, targetMin), (rangeMax, targetMax) in zip(
        ...
        log.info(
            "Sampling %u points in range %g,%g.", samples, rangeMin, rangeMax
        )
        ...
        for value in sorted(targetValues):
            ...
            targetValue = piecewiseLinearMap(
                targetMeasurement, measurementValue
            )
    ...
    if all(abs(k - v) < 0.01 for k, v in outNormalized.items()):
        log.info(
            "Detected identity mapping for the `%s` axis. Dropping.", axisTag
        )
        out = {}
        outNormalized = {}
...
def main(args=None):
    ...
    parser.add_argument(
        "font", metavar="varfont.ttf", help="Variable-font file."
    )
    parser.add_argument(
        "-o",
        "--output-file",
    ...
    parser.add_argument(
        "--weights",
        type=str,
        help="Space-separate list of weights to generate.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sizes",
        type=str,
        help="Space-separate list of optical-sizes to generate.",
    )
    ...
    configLogger(
        level=(
            "DEBUG"
            if options.verbose
            else "WARNING" if options.quiet else "INFO"
        )
    )
    ...
    if options.output_file is None:
        outfile = makeOutputFileName(
            options.font, overWrite=True, suffix=".avar"
        )
    else:
