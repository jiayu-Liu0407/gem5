...
def addFeatureVariations(font, conditionalSubstitutions, featureTag="rvrn"):
    ...
    featureTags = (
        [featureTag] if isinstance(featureTag, str) else sorted(featureTag)
    )
    ...
    if "GSUB" not in font:
        ...
        existingTags = _existingVariableFeatures(
            font["GSUB"].table
        ).intersection(featureTags)
        if existingTags:
            raise VarLibError(
    ...
    addFeatureVariationsRaw(
        font, font["GSUB"].table, conditionsAndLookups, featureTags
    )
...
def _existingVariableFeatures(table):
    ...
    if (
        hasattr(table, "FeatureVariations")
        and table.FeatureVariations is not None
    ):
        ...
        for fvr in table.FeatureVariations.FeatureVariationRecord:
            for ftsr in fvr.FeatureTableSubstitution.SubstitutionRecord:
                existingFeatureVarsTags.add(
                    features[ftsr.FeatureIndex].FeatureTag
                )
        ...
        for box, rank in boxMap.items():
            for currBox in currRegion:
                ...
                if intersection is not None:
                    ...
                    newMap[intersection] = (
                        newMap.get(intersection, 0) | rank | currRank
                    )
...
def addFeatureVariationsRaw(
    font, table, conditionalSubstitutions, featureTag="rvrn"
):
    ...
    axisIndices = {
        axis.axisTag: axisIndex
        for axisIndex, axis in enumerate(font["fvar"].axes)
    }
    ...
    for conditionSet, lookupIndices in conditionalSubstitutions:
        ...
        if hasFeatureVariations and (
            fvr := findFeatureVariationRecord(
                table.FeatureVariations, conditionTable
            )
        ):
    ...
    if hasFeatureVariations:
        ...
        table.FeatureVariations.FeatureVariationRecord.extend(
            featureVariationRecords
        )
...
def sortFeatureList(table):
    ...
    featureRemap = dict(
        zip(
            [index for tag, index, fea in tagIndexFea], range(len(tagIndexFea))
        )
    )
...
def remapFeatures(table, featureRemap):
    ...
    for scriptIndex, script in enumerate(table.ScriptList.ScriptRecord):
        ...
        for langSysRecordIndex, langSysRec in enumerate(
...
def _remapLangSys(langSys, featureRemap):
    ...
    langSys.FeatureIndex = [
        featureRemap[index] for index in langSys.FeatureIndex
    ]
