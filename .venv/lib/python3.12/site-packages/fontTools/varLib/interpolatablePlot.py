...
"""
    ...
    def draw_legend(
        self, *, show_tolerance=True, tolerance=None, kinkiness=None
    ):
        ...
        self.draw_label(
            "Point causing kink in the contour", x=xxx, y=y, width=width
        )
        self.draw_dot(
            cr,
            x=xx,
        ...
        self.draw_label(
            "Suggested new contour start point", x=xxx, y=y, width=width
        )
        self.draw_dot(
            cr,
            x=xx,
        ...
        self.draw_label(
            "Contour start point and direction", x=xxx, y=y, width=width
        )
        self.draw_arrow(
            cr,
            x=xx - self.start_arrow_length * 0.3,
    ...
    def add_summary(self, problems):
        ...
        for problem_type in sorted(
            ...
            self.draw_label(
                "%s: %d"
                % (problem_type, len(glyphs_per_problem[problem_type])),
                x=x,
                y=y,
                width=width,
                bold=True,
            ...
            for glyphname in sorted(glyphs_per_problem[problem_type]):
                ...
                self.draw_label(
                    glyphname, x=x + 2 * pad, y=y, width=width - 2 * pad
                )
                y += self.font_size
    ...
    def _add_listing(self, title, items):
        ...
        for page_no, (glyphname, problems) in items:
            ...
            if y + self.font_size > height:
                ...
                y = self.font_size + pad
            self.draw_label(
                glyphname, x=x + 5 * pad, y=y, width=width - 2 * pad
            )
    ...
    def add_index(self):
        self._add_listing(
            "Index", sorted(self.toc.items(), key=lambda x: x[1][0])
        )

    def add_problems(
        self, problems, *, show_tolerance=True, show_page_number=True
    ):
    ...
    def add_problem(
        ...
        *,
        ...
        if not all(pt == problem_type for pt in problem_types):
            problem_type = ", ".join(
                sorted({problem["type"] for problem in problems})
            )
        ...
        if problem_type == InterpolatableProblem.MISSING:
            sample_glyph = next(
                i
                for i, m in enumerate(self.glyphsets)
                if m[glyphname] is not None
            )
        ...
        for which, master_idx in enumerate(master_indices):
            ...
            if glyphset[glyphname] is not None:
                scales.append(
                    self.draw_glyph(
                        glyphset, glyphname, problems, which, x=x, y=y
                    )
                )
            else:
            ...
            for problem in problems:
                if problem["type"] == InterpolatableProblem.WRONG_START_POINT:
                    ...
                    if problem["reversed"]:
                        ...
                        points2 = new_points2
                        proposed_start = (
                            len(points2.value) - 2 - proposed_start
                        )
    ...
    def draw_glyph(
        self, glyphset, glyphname, problems, which, *, x=0, y=0, scale=None
    ):
        if type(problems) not in (list, tuple):
            problems = [problems]
        ...
        if self.fill_color or self.stroke_color:
            ...
            if (
                self.fill_color
                and problem_type != InterpolatableProblem.OPEN_PATH
            ):
                cr.set_source_rgb(*self.fill_color)
                cr.fill_preserve()
    ...
    def draw_text(
        self, text, *, x=0, y=0, color=(0, 0, 0), width=None, height=None
    ):
        if width is None:
            width = self.width
        if height is None:
