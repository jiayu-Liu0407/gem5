...
class LookupBlock(Block):
    ...
    def build(self, builder):
        builder.start_lookup_block(
            self.location, self.name, self.use_extension
        )
        Block.build(self, builder)
...
class GlyphClassDefStatement(Statement):
    ...
    def build(self, builder):
        ...
        liga = (
            self.ligatureGlyphs.glyphSet() if self.ligatureGlyphs else tuple()
        )
        mark = self.markGlyphs.glyphSet() if self.markGlyphs else tuple()
        comp = (
            self.componentGlyphs.glyphSet()
            if self.componentGlyphs
            else tuple()
        )
...
class CursivePosStatement(Statement):
    ...
    def asFea(self, indent=""):
        entry = (
            self.entryAnchor.asFea() if self.entryAnchor else "<anchor NULL>"
        )
        exit = self.exitAnchor.asFea() if self.exitAnchor else "<anchor NULL>"
        return "pos cursive {} {} {};".format(
            self.glyphclass.asFea(), entry, exit
        )
...
class LanguageStatement(Statement):
    ...
    def __init__(
        self, language, include_default=True, required=False, location=None
    ):
        Statement.__init__(self, location)
...
class LanguageSystemStatement(Statement):
    ...
    def asFea(self, indent=""):
        return "languagesystem {} {};".format(
            self.script, self.language.strip()
        )
...
class LigatureCaretByIndexStatement(Statement):
    ...
    def build(self, builder):
        ...
        glyphs = self.glyphs.glyphSet()
        builder.add_ligatureCaretByIndex_(
            self.location, glyphs, set(self.carets)
        )
...
class LigatureCaretByPosStatement(Statement):
    ...
    def build(self, builder):
        ...
        glyphs = self.glyphs.glyphSet()
        builder.add_ligatureCaretByPos_(
            self.location, glyphs, set(self.carets)
        )
...
class LigatureSubstStatement(Statement):
    ...
    def __init__(
        self, prefix, glyphs, suffix, replacement, forceChain, location=None
    ):
        Statement.__init__(self, location)
        self.prefix, self.glyphs, self.suffix = (prefix, glyphs, suffix)
...
class LookupFlagStatement(Statement):
    ...
    def build(self, builder):
        ...
        builder.set_lookup_flag(
            self.location, self.value, markAttach, markFilter
        )
    ...
    def asFea(self, indent=""):
        ...
        if self.markAttachment is not None:
            res.append(
                "MarkAttachmentType {}".format(self.markAttachment.asFea())
            )
        if self.markFilteringSet is not None:
            res.append(
                "UseMarkFilteringSet {}".format(self.markFilteringSet.asFea())
            )
        if not res:
            res = ["0"]
...
class MarkBasePosStatement(Statement):
    ...
    def build(self, builder):
        ...
        builder.add_mark_base_pos(
            self.location, self.base.glyphSet(), self.marks
        )
...
class MarkLigPosStatement(Statement):
    ...
    def build(self, builder):
        ...
        builder.add_mark_lig_pos(
            self.location, self.ligatures.glyphSet(), self.marks
        )
...
class MarkMarkPosStatement(Statement):
    ...
    def build(self, builder):
        ...
        builder.add_mark_mark_pos(
            self.location, self.baseMarks.glyphSet(), self.marks
        )
...
class ReverseChainSingleSubstStatement(Statement):
    ...
    def __init__(
        self, old_prefix, old_suffix, glyphs, replacements, location=None
    ):
        Statement.__init__(self, location)
...
class SingleSubstStatement(Statement):
    ...
    def __init__(
        self, glyphs, replace, prefix, suffix, forceChain, location=None
    ):
        Statement.__init__(self, location)
...
class SinglePosStatement(Statement):
    ...
    def build(self, builder):
        ...
        builder.add_single_pos(
            self.location, prefix, suffix, pos, self.forceChain
        )
...
class NameRecord(Statement):
    ...
    def __init__(
        self, nameID, platformID, platEncID, langID, string, location=None
    ):
        Statement.__init__(self, location)
        self.nameID = (
            nameID  #: Name ID as integer (e.g. 9 for designer's name)
        )
...
class SizeParameters(Statement):
    ...
    def __init__(
        self, DesignSize, SubfamilyID, RangeStart, RangeEnd, location=None
    ):
        Statement.__init__(self, location)
    ...
    def asFea(self, indent=""):
        ...
        if self.RangeStart != 0 or self.RangeEnd != 0:
            res += " {} {}".format(
                int(self.RangeStart * 10), int(self.RangeEnd * 10)
            )
        return res + ";"
...
class CVParametersNameStatement(NameRecord):
    ...
    def build(self, builder):
        ...
        if self.block_name == "ParamUILabelNameID":
            item = "_{}".format(
                builder.cv_num_named_params_.get(self.nameID, 0)
            )
...
class BaseAxis(Statement):
    ...
    def __init__(self, bases, scripts, vertical, minmax=None, location=None):
        ...
        self.vertical = (
            vertical  #: Boolean; VertAxis if True, HorizAxis if False
        )
        self.minmax = []  #: A set of minmax record

    def build(self, builder):
        """Calls the builder object's ``set_base_axis`` callback."""
        builder.set_base_axis(
            self.bases, self.scripts, self.vertical, self.minmax
        )
...
class STATDesignAxisStatement(Statement):
    ...
    def asFea(self, indent=""):
        ...
        res += ("\n" + indent).join(
            [s.asFea(indent=indent) for s in self.names]
        ) + "\n"
        res += "};"
        return res
