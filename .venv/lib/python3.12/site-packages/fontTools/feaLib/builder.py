...
class Builder(object):
    ...
    def build(self, tables=None, debug=False):
        ...
        for tag in ("GPOS", "GSUB"):
            ...
            if (
                ...
                del self.font[tag]
        if (
            any(tag in self.font for tag in ("GPOS", "GSUB"))
    ...
    def build_feature_aalt_(self):
        ...
        for location, name in self.aalt_features_ + [(None, "aalt")]:
            ...
            if not feature and name != "aalt":
                warnings.warn(
                    "%s: Feature %s has not been defined" % (location, name)
                )
                continue
        ...
        multi = {
            glyph: repl for glyph, repl in alternates.items() if len(repl) > 1
        }
        ...
        self.lookups_ = []
        self.start_feature(
            self.aalt_location_, "aalt", self.aalt_use_extension_
        )
        if single:
            ...
            if not isinstance(nameID, int):
                ...
                if tag in self.featureNames_:
                    if tag not in self.featureNames_ids_:
                        self.featureNames_ids_[tag] = self.get_user_name_id(
                            table
                        )
                    ...
                    if tag not in self.cv_parameters_ids_:
                        self.cv_parameters_ids_[tag] = self.get_user_name_id(
                            table
                        )
    ...
    def build_OS_2(self):
        ...
        if version >= 5:
            checkattr(
                table, ("usLowerOpticalPointSize", "usUpperOpticalPointSize")
            )
    ...
    def addDesignAxis(self, designAxis, location):
        ...
        if designAxis.axisOrder in (
            r.axisOrder for r in self.stat_["DesignAxes"]
        ):
            raise FeatureLibError(
            ...
            for avr in axisValueRecords:
                ...
                if len(avr.locations) == 1:
                    ...
                    valuesDict.update(
                        {
                            "location": {
                                i.tag: i.values[0] for i in avr.locations
                            },
                            "name": avr.names,
                        }
                    )
    ...
    def buildBASEAxis(self, axis):
        ...
        for script in sorted(scripts):
            ...
            for c in script[2]:
                record.BaseScript.BaseValues.BaseCoord.append(
                    self.buildBASECoord(c)
                )
    ...
    def buildLookups_(self, tag):
        ...
        for lookup in self.lookups_:
            ...
            resolved = lookup.promote_lookup_type(
                is_named_lookup=name is not None
            )
            if resolved is None:
                raise FeatureLibError(
                    "Within a named lookup block, all rules must be of "
            ...
            for l in resolved:
                ...
                self.lookup_locations[tag][str(lookup.lookup_index)] = (
                    LookupDebugInfo(
        ...
        for l in lookups:
            try:
                ...
                location = self.lookup_locations[tag][
                    str(l.lookup_index)
                ].location
    ...
    def makeTable(self, tag):
        ...
        for key, lookups in sorted(self.features_.items(), key=sortFeatureTag):
            ...
            if (
                len(lookup_indices) == 0
                and not size_feature
                and not force_feature
            ):
                continue

            for ix in lookup_indices:
                try:
                    self.lookup_locations[tag][str(ix)] = (
                        self.lookup_locations[tag][str(ix)]._replace(
                            feature=key
                        )
                    )
                except KeyError:
        ...
        for key, value in mapping.items():
            if key in lookup.mapping:
                if value == lookup.mapping[key]:
                    ...
                    raise FeatureLibError(
                        'Already defined substitution for "%s"'
                        % ", ".join(key),
                        location,
                    )
    ...
    def add_multiple_subst(
        ...
        if prefix or suffix or forceChain:
            self.add_multi_subst_chained_(
                location, prefix, glyph, suffix, replacements
            )
            return
        self.add_any_subst_(
            location,
    ...
    def add_ligature_subst(
        ...
        if not all(glyphs):
            raise FeatureLibError(
                "Empty glyph class in substitution", location
            )
    ...
    def add_multi_subst_chained_(
        self, location, prefix, glyph, suffix, replacements
    ):
        ...
        chain = self.get_lookup_(location, ChainContextSubstBuilder)
        sub = chain.find_chainable_subst(
            {glyph: replacements}, MultipleSubstBuilder
        )
        if sub is None:
            sub = self.get_chained_lookup_(location, MultipleSubstBuilder)
        sub.mapping[glyph] = replacements
        chain.rules.append(
            ChainContextualRule(prefix, [{glyph}], suffix, [sub])
        )
    ...
    def add_class_pair_pos(
        ...
        v1 = self.makeOpenTypeValueRecord(
            location, value1, pairPosContext=True
        )
        v2 = self.makeOpenTypeValueRecord(
            location, value2, pairPosContext=True
        )
    ...
    def add_mark_lig_pos(self, location, ligatures, components):
        ...
        for marks in components:
            ...
            for ligAnchor, markClass in marks:
                anchors[markClass.name] = self.makeOpenTypeAnchor(
                    location, ligAnchor
                )
    ...
    def add_conditionset(self, location, key, value):
        ...
        if "avar" in self.font:
            ...
            value = {
                axis: tuple(
                    (
                        piecewiseLinearMap(v, mapping[axis])
                        if axis in mapping
                        else v
                    )
    ...
    def makeAnchorPos(self, varscalar, deviceTable, location):
        ...
        if device is not None and deviceTable is not None:
            raise FeatureLibError(
                "Can't define a device coordinate and variable scalar",
                location,
            )
        return default, device
    ...
    def makeOpenTypeAnchor(self, location, anchor):
        ...
        x, deviceX = self.makeAnchorPos(
            anchor.x, anchor.xDeviceTable, location
        )
        y, deviceY = self.makeAnchorPos(
            anchor.y, anchor.yDeviceTable, location
        )
        otlanchor = otl.buildAnchor(
            x, y, anchor.contourpoint, deviceX, deviceY
        )
        return otlanchor
