...
class VariableScalar:
    ...
    def value_at_location(self, location, model_cache=None, avar=None):
        ...
        return self.model(model_cache, avar).interpolateFromMasters(
            loc, values
        )
    ...
    def model(self, model_cache=None, avar=None):
        ...
        locations = [
            dict(self._normalized_location(k)) for k in self.values.keys()
        ]
        if avar is not None:
    ...
    def add_to_variation_store(
        self, store_builder, model_cache=None, avar=None
    ):
