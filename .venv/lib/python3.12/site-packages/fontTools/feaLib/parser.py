...
class Parser(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "glyphMap" in kwargs:
            ...
            if glyphNames:
                raise TypeError(
                    "'glyphNames' and (deprecated) 'glyphMap' are "
                    "mutually exclusive"
                )
    ...
    def parse(self):
        ...
        while self.next_token_type_ is not None or self.cur_comments_:
            ...
            if self.cur_token_type_ is Lexer.COMMENT:
                ...
                self.cur_token_type_ is Lexer.SYMBOL and self.cur_token_ == ";"
            ):
                continue
            else:
    ...
    def parse_anchor_(self):
        ...
        x, y = self.expect_number_(variable=True), self.expect_number_(
            variable=True
        )
    ...
    def parse_attach_(self):
        ...
        self.expect_symbol_(";")
        return self.ast.AttachStatement(
            glyphs, contourPoints, location=location
        )
    ...
    def parse_glyphclass_(self, accept_glyphname, accept_null=False):
        ...
        if accept_glyphname and self.next_token_type_ in (
            Lexer.NAME,
            Lexer.CID,
        ):
        ...
        while self.next_token_ != "]":
            if self.next_token_type_ is Lexer.NAME:
                ...
                if (
                    "-" in glyph
                    and self.glyphNames_
                    and glyph not in self.glyphNames_
                ):
    ...
    def parse_ignore_(self):
        ...
        if self.cur_token_ in ["substitute", "sub"]:
            ...
            return self.ast.IgnoreSubstStatement(
    ...
    def parse_ligatureCaretByIndex_(self):
        ...
        self.expect_symbol_(";")
        return self.ast.LigatureCaretByIndexStatement(
            glyphs, carets, location=location
        )
    ...
    def parse_ligatureCaretByPos_(self):
        ...
        self.expect_symbol_(";")
        return self.ast.LigatureCaretByPosStatement(
            glyphs, carets, location=location
        )
    ...
    def parse_position_ligature_(self, enumerated, vertical):
        ...
        self.expect_symbol_(";")
        return self.ast.MarkLigPosStatement(
            ligatures, marks, location=location
        )
    ...
    def parse_position_mark_(self, enumerated, vertical):
        ...
        marks = self.parse_anchor_marks_()
        self.expect_symbol_(";")
        return self.ast.MarkMarkPosStatement(
            baseMarks, marks, location=location
        )
    ...
    def parse_substitute_(self):
        ...
        if self.next_token_ == "by":
            ...
            while self.next_token_ != ";":
                gc = self.parse_glyphclass_(
                    accept_glyphname=True, accept_null=True
                )
                new.append(gc)
        ...
        if not reverse and len(old) == 1 and len(new) > 1 and num_lookups == 0:
            ...
            for n in new:
                if not list(n.glyphSet()):
                    raise FeatureLibError(
                        "Empty class in replacement", location
                    )
    ...
    def parse_size_parameters_(self):
        ...
        RangeEnd = 0.0
        if (
            self.next_token_type_ in (Lexer.NUMBER, Lexer.FLOAT)
            or SubfamilyID != 0
        ):
    ...
    def parse_nameid_(self):
        ...
        if nameID > 32767:
            raise FeatureLibError(
                "Name id value cannot be greater than 32767",
                self.cur_token_location_,
            )
        platformID, platEncID, langID, string = self.parse_name_()
    ...
    def parse_STAT_ElidedFallbackName(self):
        ...
        while self.next_token_ != "}" or self.cur_comments_:
            ...
            if self.is_cur_keyword_("name"):
                ...
                if self.cur_token_ != ";":
                    raise FeatureLibError(
                        f"Unexpected token {self.cur_token_} "
    ...
    def parse_STAT_design_axis(self):
        ...
        while self.next_token_ != "}" or self.cur_comments_:
            ...
            if self.cur_token_type_ is Lexer.COMMENT:
                ...
                raise FeatureLibError(
                    f'Expected "name", got {self.cur_token_}',
                    self.cur_token_location_,
                )
    ...
    def parse_STAT_axis_value_(self):
        ...
        if not names:
            raise FeatureLibError(
                'Expected "Axis Name"', self.cur_token_location_
            )
        if not locations:
            raise FeatureLibError(
                'Expected "Axis location"', self.cur_token_location_
            )
        if len(locations) > 1:
            ...
            for location in locations:
                ...
                if tag in format4_tags:
                    raise FeatureLibError(
                        f"Axis tag {tag} already " "defined.",
                        self.cur_token_location_,
                    )
    ...
    def parse_languagesystem_(self):
        ...
        language = self.expect_language_tag_()
        self.expect_symbol_(";")
        return self.ast.LanguageSystemStatement(
            script, language, location=location
        )
    ...
    def parse_feature_block_(self, variation=False):
        ...
        self.parse_block_(
            block, vertical, stylisticset, size_feature, cv_feature
        )
        return block
    ...
    def parse_feature_reference_(self):
        ...
        self.expect_symbol_(";")
        return self.ast.FeatureReferenceStatement(
            featureName, location=location
        )
    ...
    def parse_conditionset_(self):
        ...
        while self.next_token_ != "}":
            ...
            if self.cur_token_type_ is not Lexer.NAME:
                raise FeatureLibError(
                    "Expected an axis name", self.cur_token_location_
                )

            axis = self.cur_token_
            if axis in conditions:
                raise FeatureLibError(
                    f"Repeated condition for axis {axis}",
                    self.cur_token_location_,
                )
    ...
    def parse_block_(
        ...
        while self.next_token_ != "}" or self.cur_comments_:
            ...
            if self.cur_token_type_ is Lexer.COMMENT:
                ...
                {"sub", "substitute", "rsub", "reversesub"}
            ):
    ...
    def expect_filename_(self):
        ...
        if self.cur_token_type_ is not Lexer.FILENAME:
            raise FeatureLibError(
                "Expected file name", self.cur_token_location_
            )
        return self.cur_token_
    ...
    def expect_symbol_(self, symbol):
        ...
        if self.cur_token_type_ is Lexer.SYMBOL and self.cur_token_ == symbol:
            return symbol
        raise FeatureLibError(
            "Expected '%s'" % symbol, self.cur_token_location_
        )
    ...
    def expect_keyword_(self, keyword):
        ...
        if self.cur_token_type_ is Lexer.NAME and self.cur_token_ == keyword:
            return self.cur_token_
        raise FeatureLibError(
            'Expected "%s"' % keyword, self.cur_token_location_
        )
    ...
    def expect_number_(self, variable=False):
        ...
        if (
            variable
            and self.cur_token_type_ is Lexer.SYMBOL
            and self.cur_token_ == "("
        ):
    ...
    def expect_master_(self):
        ...
        while True:
            ...
            if not (
                ...
                raise FeatureLibError(
                    "Expected an comma or an equals sign",
                    self.cur_token_location_,
                )
            self.advance_lexer_()
        self.advance_lexer_()
    ...
    def expect_any_number_(self):
        ...
        if self.cur_token_type_ in Lexer.NUMBERS:
            return self.cur_token_
        raise FeatureLibError(
            "Expected a decimal, hexadecimal or octal number",
            self.cur_token_location_,
        )
