...
timer = Timer(
    logger=logging.getLogger(__name__ + ".timer"), level=logging.INFO
)
...
class Merger(object):
    ...
    def merge(self, fontfiles):
        ...
        allTags = reduce(
            set.union, (list(font.keys()) for font in fonts), set()
        )
    ...
    def mergeObjects(self, returnTable, logic, tables):
        ...
        allKeys = set.union(
            ...
            *(
                vars(table).keys()
                for table in tables
                if table is not NotImplemented
            ),
        )
        for key in allKeys:
            log.info(" %s", key)
            ...
            if mergeLogic is NotImplemented:
                continue
            value = mergeLogic(
                getattr(table, key, NotImplemented) for table in tables
            )
