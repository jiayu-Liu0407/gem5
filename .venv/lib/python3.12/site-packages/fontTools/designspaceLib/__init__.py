...
class InstanceDescriptor(SimpleDescriptor):
    ...
    def setStyleMapStyleName(self, styleMapStyleName, languageCode="en"):
        self.localisedStyleMapStyleName[languageCode] = tostr(
            styleMapStyleName
        )
    ...
    def setStyleMapFamilyName(self, styleMapFamilyName, languageCode="en"):
        self.localisedStyleMapFamilyName[languageCode] = tostr(
            styleMapFamilyName
        )
    ...
    def getFullDesignLocation(
        ...
        for axis in doc.axes:
            if axis.name in self.designLocation:
                ...
                result[axis.name] = axis.map_forward(
                    self.userLocation[axis.name]
                )
            else:
                result[axis.name] = axis.map_forward(axis.default)
        return result

    def getFullUserLocation(
        self, doc: "DesignSpaceDocument"
    ) -> SimpleLocationDict:
...
class RangeAxisSubsetDescriptor(SimpleDescriptor):
    ...
    def __init__(
        ...
        *,
        name,
        userMinimum=-math.inf,
        userDefault=None,
        userMaximum=math.inf,
    ):
        self.name: str = name
...
class BaseDocWriter(object):
    ...
    def write(self, pretty=True, encoding="UTF-8", xml_declaration=True):
        self.root.attrib["format"] = ".".join(
            str(i) for i in self.effectiveFormatTuple
        )
        ...
        if self.documentObject.axisMappings:
            ...
            for mappingObject in self.documentObject.axisMappings:
                if (
                    getattr(mappingObject, "groupDescription", None)
                    != lastGroup
                ):
                    if mappingsElement is not None:
                        self.root.findall(".axes")[0].append(mappingsElement)
                    lastGroup = getattr(
                        mappingObject, "groupDescription", None
                    )
    ...
    def _makeLocationElement(self, locationObject, name=None):
        ...
        for dimensionName, dimensionValue in validatedLocation.items():
            ...
            if type(dimensionValue) == tuple:
                dimElement.attrib["xvalue"] = self.intOrFloat(
                    dimensionValue[0]
                )
                dimElement.attrib["yvalue"] = self.intOrFloat(
                    dimensionValue[1]
                )
            else:
    ...
    def _addAxisLabel(
        ...
        if label.userMinimum is not None:
            labelElement.attrib["userminimum"] = self.intOrFloat(
                label.userMinimum
            )
        if label.userMaximum is not None:
            labelElement.attrib["usermaximum"] = self.intOrFloat(
                label.userMaximum
            )
    ...
    def _addInstance(self, instanceObject):
        ...
        if instanceObject.localisedStyleName:
            ...
            for code in languageCodes:
                ...
                localisedStyleNameElement.text = instanceObject.getStyleName(
                    code
                )
        ...
        if instanceObject.localisedFamilyName:
            ...
            for code in languageCodes:
                ...
                localisedFamilyNameElement.text = instanceObject.getFamilyName(
                    code
                )
                instanceElement.append(localisedFamilyNameElement)
        if instanceObject.localisedStyleMapStyleName:
            languageCodes = list(
                instanceObject.localisedStyleMapStyleName.keys()
            )
            languageCodes.sort()
            for code in languageCodes:
                if code == "en":
                    continue
                localisedStyleMapStyleNameElement = ET.Element(
                    "stylemapstylename"
                )
        ...
        if instanceObject.localisedStyleMapFamilyName:
            languageCodes = list(
                instanceObject.localisedStyleMapFamilyName.keys()
            )
            languageCodes.sort()
            for code in languageCodes:
                if code == "en":
                    continue
                localisedStyleMapFamilyNameElement = ET.Element(
                    "stylemapfamilyname"
                )
        ...
        if self.effectiveFormatTuple >= (5, 0):
            ...
            if instanceObject.location is not None:
                ...
                ) = self._makeLocationElement(instanceObject.location)
    ...
    def _addSource(self, sourceObject):
        ...
        if sourceObject.localisedFamilyName:
            ...
            for code in languageCodes:
                ...
                localisedFamilyNameElement = ET.Element("familyname")
                localisedFamilyNameElement.attrib[XML_LANG] = code
                localisedFamilyNameElement.text = sourceObject.getFamilyName(
                    code
                )
    ...
    def _addLib(
        self, parentElement: ET.Element, data: Any, indent_level: int
    ) -> None:
        if not data:
            return
        libElement = ET.Element("lib")
    ...
    def _writeGlyphElement(
        self, instanceElement, instanceObject, glyphName, data
    ):
        glyphElement = ET.Element("glyph")
        if data.get("mute"):
            glyphElement.attrib["mute"] = "1"
        ...
        if data.get("instanceLocation") is not None:
            (
                locationElement,
                data["instanceLocation"],
            ) = self._makeLocationElement(data.get("instanceLocation"))
...
class BaseDocReader(LogMixin):
    ...
    def __init__(self, documentPath, documentObject):
        ...
        self.documentObject.formatVersion = self.root.attrib.get(
            "format", "3.0"
        )
        self._axes = []
        self.rules = []
    ...
    def readAxes(self):
        ...
        if (
            axesElement is not None
            and "elidedfallbackname" in axesElement.attrib
        ):
        ...
        for axisElement in axisElements:
            ...
            for labelNameElement in axisElement.findall("labelname"):
                ...
                for key, lang in labelNameElement.items():
                    if key == XML_LANG:
                        axisObject.labelNames[lang] = tostr(
                            labelNameElement.text
                        )
            labelElement = axisElement.find(".labels")
            if labelElement is not None:
                if "ordering" in labelElement.attrib:
                    axisObject.axisOrdering = int(
                        labelElement.attrib["ordering"]
                    )
    ...
    def readAxisLabel(self, element: ET.Element):
        ...
        linkedValue = (
            float(linkedValueStr) if linkedValueStr is not None else None
        )
    ...
    def readLabels(self):
        ...
        for labelElement in self.root.findall(".labels/label"):
            ...
            designLocation, userLocation = self.locationFromElement(
                labelElement
            )
            if designLocation:
                raise DesignSpaceDocumentError(
            ...
            elidable = (
                True if labelElement.get("elidable") == "true" else False
            )
            olderSibling = (
                True if labelElement.get("oldersibling") == "true" else False
            )
            labelNames = {
                lang: label_name.text or ""
    ...
    def readVariableFonts(self):
        ...
        for variableFontElement in self.root.findall(
    ...
    def readAxisSubset(self, element: ET.Element):
        if "uservalue" in element.attrib:
            ...
            return self.valueAxisSubsetDescriptorClass(
                name=name, userValue=userValue
            )
        else:
    ...
    def readSources(self):
        for sourceCount, sourceElement in enumerate(
            ...
            sourceObject.filename = (
                filename  # path as it is stored in the document
            )
    ...
    def readLocationElement(self, locationElement):
        ...
        for dimensionElement in locationElement.findall(".dimension"):
            ...
            if self._strictAxisNames and dimName not in self.axisDefaults:
                ...
                self.log.warning(
                    'Location with undefined axis: "%s".', dimName
                )
                continue
            ...
            try:
                ...
                self.log.warning(
                    "ValueError in readLocation xValue %3.3f", xValue
                )
            try:
                ...
                self.log.warning(
                    "ValueError in readLocation yValue %3.3f", yValue
                )
    ...
    def _readSingleInstanceElement(
        ...
        for styleMapStyleNameElement in instanceElement.findall(
            "stylemapstylename"
        ):
            for key, lang in styleMapStyleNameElement.items():
                if key == XML_LANG:
                    styleMapStyleName = styleMapStyleNameElement.text
                    instanceObject.setStyleMapStyleName(
                        styleMapStyleName, lang
                    )
        for styleMapFamilyNameElement in instanceElement.findall(
            "stylemapfamilyname"
        ):
            for key, lang in styleMapFamilyNameElement.items():
                if key == XML_LANG:
                    styleMapFamilyName = styleMapFamilyNameElement.text
                    instanceObject.setStyleMapFamilyName(
                        styleMapFamilyName, lang
                    )
        designLocation, userLocation = self.locationFromElement(
            instanceElement
        )
    ...
    def readGlyphElement(self, glyphElement, instanceObject):
        ...
        if glyphName is None:
            raise DesignSpaceDocumentError(
                "Glyph object without name attribute"
            )
...
class DesignSpaceDocument(LogMixin, AsDictMixin):
    ...
    **Note:** Python attribute names are usually camelCased, the
    ...
    def addLocationLabel(
        self, locationLabelDescriptor: LocationLabelDescriptor
    ):
    ...
    def updateFilenameFromPath(
        self, masters=True, instances=True, force=False
    ):
    ...
    def getAxis(
        self, name: str
    ) -> AxisDescriptor | DiscreteAxisDescriptor | None:
    ...
    def map_forward(
        ...
        """
        return {
            axis.name: axis.map_forward(
                userLocation.get(axis.name, axis.default)
            )
    ...
    def normalizeLocation(self, location):
        ...
        for axis in self.axes:
            ...
            triple = [
                axis.map_forward(v)
                for v in (axis.minimum, axis.default, axis.maximum)
            ]
    ...
    def normalize(self):
        ...
        for axis in self.axes:
            ...
            for inputValue, outputValue in axis.map:
                newOutputValue = self.normalizeLocation(
                    {axis.name: outputValue}
                ).get(axis.name)
                newMap.append((inputValue, newOutputValue))
            if newMap:
                axis.map = newMap
            # finally the axis values
            minimum = self.normalizeLocation({axis.name: axis.minimum}).get(
                axis.name
            )
            maximum = self.normalizeLocation({axis.name: axis.maximum}).get(
                axis.name
            )
            default = self.normalizeLocation({axis.name: axis.default}).get(
                axis.name
            )
            ...
            for conditions in rule.conditionSets:
                ...
                for cond in conditions:
                    ...
                    newConditions.append(
                        dict(
                            name=cond["name"], minimum=minimum, maximum=maximum
                        )
                    )
    ...
    def getVariableFonts(self) -> List[VariableFontDescriptor]:
        ...
        for axis in self.axes:
            if hasattr(axis, "values"):
                ...
                rangeAxisSubsets.append(
                    RangeAxisSubsetDescriptor(name=axis.name)
                )
        valueCombinations = itertools.product(
            *[axis.values for axis in discreteAxes]
        )
        for values in valueCombinations:
            ...
            if self.path is not None:
                basename = (
                    os.path.splitext(os.path.basename(self.path))[0] + "-VF"
                )
            if basename is None:
                basename = "VF"
            axisNames = "".join(
            ...
            variableFonts.append(
                VariableFontDescriptor(
                    ...
                    + [
                        ValueAxisSubsetDescriptor(
                            name=axis.name, userValue=value
                        )
                        for axis, value in zip(discreteAxes, values)
                    ],
                )
