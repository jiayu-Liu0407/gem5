...
def _extractSubSpace(
    ...
    *,
    ...
    for mapping in doc.axisMappings:
        if not all(
            axis in subDocAxes for axis in mapping.inputLocation.keys()
        ):
            continue
        if not all(
            axis in subDocAxes for axis in mapping.outputLocation.keys()
        ):
            LOGGER.error(
        ...
        for axis in doc.axes:
            ...
            if (
                ...
                or (
                    range.default is not None and range.default != axis.default
                )
    ...
    for source in doc.sources:
        if not locationInRegion(
            doc.map_backward(source.designLocation), userRegion
        ):
            continue
        ...
        if makeNames:
            ...
            subDoc.addInstance(
                InstanceDescriptor(
                    ...
                    userLocation=(
                        {} if expandLocations else instance.userLocation
                    ),
                    designLocation=_filterLocation(
            ...
            for selectionName, selectionValue in designRegion.items():
                ...
                if isinstance(selectionValue, (float, int)):  # is point
                    ...
                    if selectionValue in cs[selectionName]:
                        ...
                        intersection = cs[selectionName].intersection(
                            selectionValue
                        )
                        # Case C-AR-inter
