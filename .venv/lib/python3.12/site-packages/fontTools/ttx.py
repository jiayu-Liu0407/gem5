...
class Options(object):
    ...
    def __init__(self, rawOptions, numFiles):
        ...
        if self.verbose and self.quiet:
            raise getopt.GetoptError(
                "-q and -v options are mutually exclusive"
            )
        if self.verbose:
        ...
        if self.mergeFile and self.flavor:
            raise getopt.GetoptError(
                "-m and --flavor options are mutually exclusive"
            )
        ...
        if self.flavor != "woff" and self.useZopfli:
            raise getopt.GetoptError(
                "--with-zopfli option requires --flavor 'woff'"
            )
...
def ttCompile(input, output, options):
    ...
    if (
        options.recalcTimestamp is None
        and "head" in ttf
        and input is not sys.stdin
    ):
...
def parseOptions(args):
    ...
    for input in files:
        ...
        if options.outputFile:
            ...
            if input == "-":
                raise getopt.GetoptError(
                    "Must provide -o when reading from stdin"
                )
