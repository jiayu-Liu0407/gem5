...
ps_special = (
    b"()<>[]{}%"  # / is one too, but we take care of that one differently
)
...
class PSInterpreter(PSOperators):
        ...
        try:
            ...
            if self.tokenizer is not None:
                log.debug(
                    ...
                    "- - - - - - -",
                    self.tokenizer.buf[
                        self.tokenizer.pos - 50 : self.tokenizer.pos
                    ],
                    self.tokenizer.buf[
                        self.tokenizer.pos : self.tokenizer.pos + 50
                    ],
                )
            raise
    ...
    def handle_object(self, object):
        if not (
            self.proclevel or object.literal or object.type == "proceduretype"
        ):
    ...
    def pop(self, *types):
        ...
        if types:
            if object.type not in types:
                raise PSError(
                    "typecheck, expected %s, found %s"
                    % (repr(types), object.type)
                )
        del stack[-1]
        return object
