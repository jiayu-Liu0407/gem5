...
class Data:
    ...
    def asBase64(
        self, maxlinelength: int = 76, indent_level: int = 1
    ) -> bytes:
...
def end_key(self: PlistTarget) -> None:
    if self.current_key or not isinstance(
        self.stack[-1], collections.abc.Mapping
    ):
...
def _string_or_data_element(
    raw_bytes: bytes, ctx: SimpleNamespace
) -> etree.Element:
...
@singledispatch
def _make_element(
    value: PlistEncodable, ctx: SimpleNamespace
) -> etree.Element:
