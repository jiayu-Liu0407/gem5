...
def _calcCubicArcLengthCRecurse(mult, p0, p1, p2, p3):
    ...
    if arch * mult + EPSILON >= box:
        ...
        return _calcCubicArcLengthCRecurse(
            mult, *one
        ) + _calcCubicArcLengthCRecurse(mult, *two)
...
def approximateQuadraticArcLength(pt1, pt2, pt3):
    ...
    """
    return approximateQuadraticArcLengthC(
        complex(*pt1), complex(*pt2), complex(*pt3)
    )
...
def approximateQuadraticArcLengthC(pt1, pt2, pt3):
    ...
    v0 = abs(
        -0.492943519233745 * pt1
        + 0.430331482911935 * pt2
        + 0.0626120363218102 * pt3
    )
    v1 = abs(pt3 - pt1) * 0.4444444444444444
    v2 = abs(
        -0.0626120363218102 * pt1
        - 0.430331482911935 * pt2
        + 0.492943519233745 * pt3
    )
...
@cython.locals(
    t2=cython.double,
    _1_t=cython.double,
    _1_t_2=cython.double,
    _2_t_1_t=cython.double,
)
...
def lineLineIntersections(s1, e1, s2, e2):
    ...
    e2x, e2y = e2
    if (
        math.isclose(s2x, e2x)
        and math.isclose(s1x, e1x)
        and not math.isclose(s1x, s2x)
    ):  # Parallel vertical
        return []
    if (
        math.isclose(s2y, e2y)
        and math.isclose(s1y, e1y)
        and not math.isclose(s1y, s2y)
    ):  # Parallel horizontal
        return []
    if math.isclose(s2x, e2x) and math.isclose(
        s2y, e2y
    ):  # Line segment is tiny
        return []
    if math.isclose(s1x, e1x) and math.isclose(
        s1y, e1y
    ):  # Line segment is tiny
        return []
    if math.isclose(e1x, s1x):
        ...
        return [
            Intersection(
                pt=pt,
                t1=_line_t_of_pt(s1, e1, pt),
                t2=_line_t_of_pt(s2, e2, pt),
            )
        ]
...
def segmentSegmentIntersections(seg1, seg2):
    ...
    if len(seg1) > 2:
        ...
        raise ValueError(
            "Couldn't work out which intersection function to use"
        )
    if not swapped:
        return intersections
