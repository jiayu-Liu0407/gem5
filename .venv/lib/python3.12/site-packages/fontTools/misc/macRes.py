...
class ResourceReader(MutableMapping):
    ...
    def _read(self, numBytes, offset=None):
        if offset is not None:
            try:
                ...
                raise ResourceError(
                    "Failed to seek offset ('offset' is too large)"
                )
        ...
        try:
            ...
            raise ResourceError(
                "Cannot read resource ('numBytes' is too large)"
            )
    ...
    def _readTypeList(self):
        ...
        for i in range(self.numTypes + 1):
            ...
            resTypeItemData = self._read(
                ResourceTypeItemSize, resTypeItemOffset
            )
    ...
    def getNames(self, resType):
        ...
        return [
            res.name for res in self.get(resType, []) if res.name is not None
        ]
...
class Resource(object):
    ...
    def decompile(self, refData, reader):
        ...
        (self.dataOffset,) = struct.unpack(
            ">L", bytesjoin([b"\0", self.dataOffset])
        )
