...
class T1Font(object):
    ...
    def encode_eexec(self, eexec_dict):
        ...
        sortedItems = sorted(
            eexec_dict.items(), key=lambda item: item[0] != "Private"
        )
            ...
            if key == "Private":
                ...
                for subkey, subvalue in pr.items():
                    ...
                    if subkey == "OtherSubrs":
                        ...
                        lines.append(
                            f"/Subrs {len(subrs)} array".encode("ascii")
                        )
                ...
                lines.append(
                    self._tobytes(
                        f"{len(eexec_dict['CharStrings'])} dict dup begin"
                    )
                )
        ...
        encrypted_eexec, R = eexec.encrypt(
            bytesjoin([eexec_IV, eexec_portion]), 55665
        )
    ...
    def _make_lines(self, key, value):
        ...
        if key in ["isFixedPitch", "ForceBold", "RndStemUp"]:
            return [
                self._tobytes(f"/{key} {'true' if value else 'false'} def")
            ]
        elif key == "Encoding":
        ...
        if isinstance(value, str):
            ...
            return [
                self._tobytes(
                    f"/{key} [{' '.join(str(v) for v in value)}] def"
                )
            ]
    ...
    for isEncrypted, chunk in chunks:
        if isEncrypted:
            ...
            if (
                ...
                and decrypted[-len(EEXECINTERNALEND) - 2 : -2]
                != EEXECINTERNALEND
            ):
