...
class TTFont(object):
    ...
    def __init__(
        ...
        self.reader = None
        self.cfg = (
            cfg.copy() if isinstance(cfg, AbstractConfig) else Config(cfg)
        )
    ...
    def _saveXML(
        ...
        if writeVersion:
            ...
            writer.begintag(
                "ttFont", sfntVersion=repr(tostr(self.sfntVersion))[1:-1]
            )
        writer.newline()
        ...
        for i in range(numTables):
            ...
            if splitTables:
                ...
                tableWriter.newline()
                writer.simpletag(
                    tagToXML(tag), src=os.path.basename(tablePath)
                )
                writer.newline()
            else:
                tableWriter = writer
    ...
    def getGlyphSet(
        ...
        glyphSet = None
        if ("CFF " in self or "CFF2" in self) and (
            preferCFF or "glyf" not in self
        ):
            glyphSet = _TTGlyphSetCFF(self, location)
        elif "glyf" in self:
            glyphSet = _TTGlyphSetGlyf(
                self, location, recalcBounds=recalcBounds
            )
        else:
