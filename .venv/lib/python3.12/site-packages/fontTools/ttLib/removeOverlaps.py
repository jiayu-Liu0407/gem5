...
def componentsOverlap(
    glyph: _g_l_y_f.Glyph, glyphSet: _TTGlyphMapping
) -> bool:
...
def _same_path(path1: pathops.Path, path2: pathops.Path) -> bool:
    return {tuple(c) for c in path1.contours} == {
        tuple(c) for c in path2.contours
    }
    ...
    *,
    ...
    glyphNames = sorted(
        ...
        key=lambda name: (
            (
                glyfTable[name]
                .getCompositeMaxpValues(glyfTable)
                .maxComponentDepth
...
def _remove_charstring_overlaps(
    *,
    ...
    if not _same_path(path, path2):
        ...
        charStrings[glyphName] = _charString_from_SkPath(
            path2, charStrings[glyphName]
        )
        return True
...
def main(args=None):
    ...
    parser.add_argument(
        "output", metavar="OUTPUT.ttf", help="Output font file"
    )
    parser.add_argument(
        "glyphs",
        metavar="GLYPHS",
