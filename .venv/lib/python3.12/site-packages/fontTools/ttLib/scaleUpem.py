        ...
        for component in g.components:
            ...
            if component.axisValuesVarIndex != otTables.NO_VARIATION_INDEX:
                ...
                if varIdx != otTables.NO_VARIATION_INDEX:
                    ...
                    storeBuilder.setSupports(
                        store.get_supports(major, fvar.axes)
                    )
                    if vec:
                        m = len(vec) // varData.VarRegionCount
                        vec = list(batched(vec, m))
                        vec = [Vector(v) for v in vec]
                        component.axisValuesVarIndex = (
                            storeBuilder.storeDeltas(vec)
                        )
                    else:
                        component.axisValuesVarIndex = (
                            otTables.NO_VARIATION_INDEX
                        )
                ...
                if varIdx != otTables.NO_VARIATION_INDEX:
                    ...
                    if vec:
                        ...
                        for v in vec:
                            ...
                            if (
                                flags
                                & otTables.VarComponentFlags.HAVE_TRANSLATE_X
                            ):
                                v[i] = visitor.scale(v[i])
                                i += 1
                            if (
                                flags
                                & otTables.VarComponentFlags.HAVE_TRANSLATE_Y
                            ):
                                v[i] = visitor.scale(v[i])
                                i += 1
                            if (
                                flags
                                & otTables.VarComponentFlags.HAVE_ROTATION
                            ):
                                i += 1
                            ...
                            if (
                                flags
                                & otTables.VarComponentFlags.HAVE_TCENTER_X
                            ):
                                v[i] = visitor.scale(v[i])
                                i += 1
                            if (
                                flags
                                & otTables.VarComponentFlags.HAVE_TCENTER_Y
                            ):
                                v[i] = visitor.scale(v[i])
                                i += 1
                        ...
                        component.transformVarIndex = storeBuilder.storeDeltas(
                            vec
                        )
                    else:
                        component.transformVarIndex = (
                            otTables.NO_VARIATION_INDEX
                        )
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Change the units-per-EM of fonts",
    )
