...
class _TTGlyphSetGlyf(_TTGlyphSet):
    def __init__(self, font, location, recalcBounds=True):
        ...
        super().__init__(
            font, location, self.glyfTable, recalcBounds=recalcBounds
        )
    ...
    if condition.Format == 1:
        ...
        for subcondition in condition.ConditionTable:
            if not _evaluateCondition(
                subcondition, fvarAxes, location, instancer
            ):
                return False
        return True
    ...
    def _draw(self, pen, isPointPen):
        ...
        for comp in glyph.components:
            if comp.flags & VarComponentFlags.HAVE_CONDITION:
                condition = varc.ConditionList.ConditionTable[
                    comp.conditionIndex
                ]
                if not _evaluateCondition(
...
def _setCoordinates(glyph, coord, glyfTable, *, recalcBounds=True):
    ...
    if glyph.isComposite():
        ...
        glyph.components = [
            copy(comp) for comp in glyph.components
        ]  # Shallow copy
...
class LerpGlyph:
    ...
    def draw(self, pen):
        ...
        replayRecording(
            lerpRecordings(recording1.value, recording2.value, factor), pen
        )
