...
class table__g_l_y_f(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        self.axisTags = (
            [axis.axisTag for axis in ttFont["fvar"].axes]
            if "fvar" in ttFont
            else []
        )
        loca = ttFont["loca"]
    ...
    def toXML(self, writer, ttFont, splitGlyphs=False):
        ...
        for glyphName in glyphNames:
            ...
            if glyph is None:
                log.warning(
                    "glyph '%s' does not exist in glyf table", glyphName
                )
                continue
            if glyph.numberOfContours:
                if splitGlyphs:
                    ...
                    glyphWriter.newline()
                    writer.simpletag(
                        "TTGlyph", src=os.path.basename(glyphPath)
                    )
                else:
                    glyphWriter = writer
    ...
    def _setCoordinates(self, glyphName, coord, hMetrics, vMetrics=None):
        ...
        if vMetrics is not None:
            ...
            ):  # unlikely but do the same as horizontal
                verticalAdvanceWidth = 0
    ...
    @deprecateFunction(
        "use '_getPhantomPoints' instead", category=DeprecationWarning
    )
    def getPhantomPoints(self, glyphName, ttFont, defaultVerticalOrigin=None):
        ...
        vMetrics = self._synthesizeVMetrics(
            glyphName, ttFont, defaultVerticalOrigin
        )
    ...
    def getCoordinatesAndControls(
        self, glyphName, ttFont, defaultVerticalOrigin=None
    ):
    ...
    @deprecateFunction(
        "use '_setCoordinates' instead", category=DeprecationWarning
    )
...
ARG_1_AND_2_ARE_WORDS = (
    0x0001  # if set args are words otherwise they are bytes
)
ARGS_ARE_XY_VALUES = (
    0x0002  # if set args are xy values, otherwise they are points
)
...
class Glyph(object):
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if name == "contour":
            ...
            for element in content:
                ...
                coordinates.append(
                    (safeEval(attrs["x"]), safeEval(attrs["y"]))
                )
    ...
    def recalcBounds(self, glyfTable, *, boundsDone=None):
        ...
        if self.isComposite() and self.tryRecalcBoundsComposite(
            ...
            return
        try:
            coords, endPts, flags = self.getCoordinates(
                glyfTable, round=otRound
            )
    ...
    def isComposite(self):
        ...
        if hasattr(self, "data"):
            return (
                struct.unpack(">h", self.data[:2])[0] == -1
                if self.data
                else False
            )
        else:
        ...
        for end in endPts:
            ...
            if 1 not in cFlags:
                ...
                if cubic:
                    ...
                    f = contour[0]
                    p0 = (
                        maybeInt((l[0] + f[0]) * 0.5),
                        maybeInt((l[1] + f[1]) * 0.5),
                    )
                    pen.moveTo(p0)
        ...
        for end in endPts:
            ...
            for i, pt in enumerate(contour):
                if cFlags[i] & flagOnCurve:
                    ...
                    pen.addPoint(pt)
                    segmentType = (
                        "curve" if cFlags[i] & flagCubic else "qcurve"
                    )
            pen.endPath()
...
def _is_mid_point(p0: tuple, p1: tuple, p2: tuple) -> bool:
    ...
    p1 = Vector(p1)
    p2 = Vector(p2)
    return ((p0 + p2) * 0.5).isclose(p1) or _roundv(p0) + _roundv(
        p2
    ) == _roundv(p1) * 2
...
class GlyphComponent(object):
    ...
    def decompile(self, data, glyfTable):
        ...
        if self.flags & WE_HAVE_A_SCALE:
            ...
            (xscale, scale01, scale10, yscale) = struct.unpack(
                ">hhhh", data[:8]
            )
            self.transform = [
    ...
    def compile(self, more, haveInstructions, glyfTable):
        ...
        if hasattr(self, "transform"):
            ...
            if transform[0][1] or transform[1][0]:
                ...
                data = data + struct.pack(
                    ">hh", transform[0][0], transform[1][1]
                )
            else:
    ...
    def toXML(self, writer, ttFont):
        ...
        if not hasattr(self, "firstPt"):
            ...
            attrs = attrs + [
                ("firstPt", self.firstPt),
                ("secondPt", self.secondPt),
            ]
