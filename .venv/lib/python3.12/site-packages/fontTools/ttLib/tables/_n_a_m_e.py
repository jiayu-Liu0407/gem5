...
class table__n_a_m_e(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        for name in names:
            ...
            if string in done:
                ...
                name.offset, name.length = done[string] = len(stringData), len(
                    string
                )
    ...
    def setName(self, string, nameID, platformID, platEncID, langID):
        ...
        if not isinstance(string, str):
            if isinstance(string, bytes):
                log.warning(
                    "name string is bytes, ensure it's correctly encoded: %r",
                    string,
                )
            else:
                raise TypeError(
        ...
        if namerecord:
            ...
            self.names.append(
                makeName(string, nameID, platformID, platEncID, langID)
            )

    def removeNames(
        self, nameID=None, platformID=None, platEncID=None, langID=None
    ):
    ...
    def addName(
        self, string, platforms=((1, 0, 0), (3, 1, 0x409)), minNameID=255
    ):
...
class NameRecord(object):
    def getEncoding(self, default="ascii"):
        ...
        """
        return getEncoding(
            self.platformID, self.platEncID, self.langID, default
        )

    def encodingIsUnicodeCompatible(self):
        return self.getEncoding(None) in [
            "utf_16_be",
            "ucs2be",
            "ascii",
            "latin1",
    ...
    def toUnicode(self, errors="strict"):
        ...
        if (
            ...
            if byteord(string[-1]) == 0:
                ...
                string = bytesjoin(
                    b"\0" + bytechr(byteord(b)) for b in string[1:]
                )
...
_MAC_LANGUAGE_CODES = {
    lang.lower(): code for code, lang in _MAC_LANGUAGES.items()
}
