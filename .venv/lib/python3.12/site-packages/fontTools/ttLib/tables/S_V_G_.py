    ...
    def decompile(self, data, ttFont):
        ...
        if self.numEntries > 0:
            ...
            for entry in entries:
                ...
                self.docList.append(
                    SVGDocument(
                        doc, entry.startGlyphID, entry.endGlyphID, compressed
                    )
                )
    ...
    def compile(self, ttFont):
        ...
        for i, doc in enumerate(self.docList):
            ...
            if (allCompressed or doc.compressed) and not docBytes.startswith(
                ...
                with gzip.GzipFile(
                    None, "w", fileobj=bytesIO, mtime=0
                ) as gzipper:
    ...
    def toXML(self, writer, ttFont):
        for i, doc in enumerate(self.docList):
            if isinstance(doc, (list, tuple)):
                ...
                self.docList[i] = doc
            attrs = {
                "startGlyphID": doc.startGlyphID,
                "endGlyphID": doc.endGlyphID,
            }
            if doc.compressed:
                attrs["compressed"] = 1
