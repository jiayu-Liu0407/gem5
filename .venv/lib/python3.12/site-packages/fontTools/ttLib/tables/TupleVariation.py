...
class TupleVariation(object):
    ...
    def __eq__(self, other):
        return (
            self.coordinates == other.coordinates and self.axes == other.axes
        )
    ...
    def getUsedPoints(self):
        ...
        if None not in self.coordinates:
            return frozenset()
        used = frozenset(
            [i for i, p in enumerate(self.coordinates) if p is not None]
        )
        ...
        for axis in axisTags:
            ...
            if value is not None:
                ...
                if minValue == defaultMinValue and maxValue == defaultMaxValue:
                    writer.simpletag(
                        "coord", axis=axis, value=fl2str(value, 14)
                    )
                else:
                    attrs = [
                        ("axis", axis),
    ...
    def fromXML(self, name, attrs, _content):
        if name == "coord":
            ...
            if "pt" in attrs:
                ...
                log.warning(
                    "bad delta format: %s" % ", ".join(sorted(attrs.keys()))
                )
    ...
    def decompileCoord_(axisTags, data, offset):
        ...
        for axis in axisTags:
            coord[axis] = fi2fl(
                struct.unpack(">h", data[pos : pos + 2])[0], 14
            )
            pos += 2
        return coord, pos
    ...
    def decompilePoints_(numPoints, data, offset, tableTag):
        ...
        if (numPointsInData & POINTS_ARE_WORDS) != 0:
            numPointsInData = (
                numPointsInData & POINT_RUN_COUNT_MASK
            ) << 8 | data[pos]
            pos += 1
    ...
    def compileDeltaValues_(deltas, bytearr=None, *, optimizeSize=True):
        ...
        if optimizeSize:
            while pos < numDeltas:
                value = deltas[pos]
                if value == 0:
                    pos = TupleVariation.encodeDeltaRunAsZeroes_(
                        deltas, pos, bytearr
                    )
                elif -128 <= value <= 127:
...
def compileTupleVariationStore(
    ...
    *,
    ...
    compiledPoints = {
        pointSet: TupleVariation.compilePoints(pointSet)
