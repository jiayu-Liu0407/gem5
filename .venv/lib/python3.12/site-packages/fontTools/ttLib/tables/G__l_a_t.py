...
class table_G__l_a_t(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        if self.version <= 1.9:
            decoder = partial(
                self.decompileAttributes12, fmt=Glat_format_1_entry
            )
        elif self.version <= 2.9:
            decoder = partial(
                self.decompileAttributes12, fmt=Glat_format_23_entry
            )
    ...
    def decompileAttributes3(self, data):
        if self.hasOctaboxes:
            o, data = sstruct.unpack2(
                Glat_format_3_octabox_metrics, data, _Object()
            )
    ...
    def compile(self, ttFont):
        ...
        if self.version <= 1.9:
            encoder = partial(
                self.compileAttributes12, fmt=Glat_format_1_entry
            )
        elif self.version <= 2.9:
            encoder = partial(
                self.compileAttributes12, fmt=Glat_format_1_entry
            ...
            self.compression = (self.scheme << 27) + (
                1 if self.hasOctaboxes else 0
            )
    ...
    def toXML(self, writer, ttFont):
        writer.simpletag(
            "version", version=self.version, compressionScheme=self.scheme
        )
        writer.newline()
        for n, a in sorted(
        ...
        for element in content:
            ...
            if tag == "attribute":
                ...
                for element in subcontent:
                    ...
                    for k, v in attrs.items():
                        setattr(
                            so, k, int(float(v[:-1]) * 255.0 / 100.0 + 0.5)
                        )
