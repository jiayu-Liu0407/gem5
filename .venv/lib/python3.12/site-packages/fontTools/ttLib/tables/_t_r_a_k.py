...
class table__t_r_a_k(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        self.reserved = 0
        tableData = bytesjoin(
            [sstruct.pack(TRAK_HEADER_FORMAT, self)] + dataList
        )
        return tableData

    def decompile(self, data, ttFont):
        sstruct.unpack(
            TRAK_HEADER_FORMAT, data[:TRAK_HEADER_FORMAT_SIZE], self
        )
        for direction in ("horiz", "vert"):
            trackData = TrackData()
...
class TrackData(MutableMapping):
    ...
    def compile(self, offset):
        ...
        offset += (
            TRACK_DATA_FORMAT_SIZE + TRACK_TABLE_ENTRY_FORMAT_SIZE * nTracks
        )
    ...
    def decompile(self, data, offset):
        ...
        for i in range(self.nTracks):
            ...
            for j in range(nSizes):
                ...
                (perSizeValue,) = struct.unpack(
                    PER_SIZE_VALUE_FORMAT, perSizeValueData
                )
...
class TrackTableEntry(MutableMapping):
    ...
    def __repr__(self):
        return "TrackTableEntry({}, nameIndex={})".format(
            self._map, self.nameIndex
        )
