...
class table_E_B_D_T_(DefaultTable.DefaultTable):
    ...
    def toXML(self, writer, ttFont):
        ...
        if ttFont.bitmapGlyphDataFormat in ("row", "bitwise"):
            locator = ttFont[self.__class__.locatorName]
            for curStrike, curGlyphDict in zip(
                locator.strikes, self.strikeData
            ):
                for curIndexSubTable in curStrike.indexSubTables:
                    for curName in curIndexSubTable.names:
                        glyph = curGlyphDict[curName]
                        ...
                        glyph.exportBitDepth = (
                            curStrike.bitmapSizeTable.bitDepth
                        )
        ...
        if name == "header":
            ...
            for element in content:
                ...
                if name[4:].startswith(_bitmapGlyphSubclassPrefix[4:]):
                    ...
                    log.warning(
                        "%s being ignored by %s", name, self.__class__.__name__
                    )
            ...
            if strikeIndex >= len(self.strikeData):
                self.strikeData += [None] * (
                    strikeIndex + 1 - len(self.strikeData)
                )
            assert (
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for element in content:
            ...
            if name in componentNames:
                ...
                log.warning(
                    "unknown name '%s' being ignored by EbdtComponent.", name
                )
...
def _writeRowImageData(strikeIndex, glyphName, bitmapObject, writer, ttFont):
    ...
    for curRow in range(metrics.height):
        rowData = bitmapObject.getRow(
            curRow, bitDepth=bitDepth, metrics=metrics
        )
...
def _writeBitwiseImageData(
    strikeIndex, glyphName, bitmapObject, writer, ttFont
):
    metrics = bitmapObject.exportMetrics
    del bitmapObject.exportMetrics
    bitDepth = bitmapObject.exportBitDepth
...
class BitmapGlyph(object):
    ...
    def getFormat(self):
        return safeEval(
            self.__class__.__name__[len(_bitmapGlyphSubclassPrefix) :]
        )
...
class BitAlignedBitmapMixin(object):
    ...
    def getRow(self, row, bitDepth=1, metrics=None, reverseBytes=False):
        ...
        assert (
            0 <= row and row < metrics.height
        ), "Illegal row access in bitmap"
    ...
    def setRows(self, dataRows, bitDepth=1, metrics=None, reverseBytes=False):
        ...
        numBytes = (
            self._getBitRange(len(dataRows), bitDepth, metrics)[0] + 7
        ) // 8
...
class ebdt_bitmap_format_1(
    ...
    def decompile(self):
        ...
        dummy, data = sstruct.unpack2(
            smallGlyphMetricsFormat, self.data, self.metrics
        )
        self.imageData = data
...
class ebdt_bitmap_format_6(
    ...
    def decompile(self):
        ...
        dummy, data = sstruct.unpack2(
            bigGlyphMetricsFormat, self.data, self.metrics
        )
        self.imageData = data
            ...
            if name == "components":
                ...
                for compElement in content:
                    ...
                    if name == "ebdtComponent":
                        ...
                        log.warning(
                            "'%s' being ignored in component array.", name
                        )
...
class ebdt_bitmap_format_8(BitmapPlusSmallMetricsMixin, ComponentBitmapGlyph):
    def decompile(self):
        ...
        for i in range(numComponents):
            curComponent = EbdtComponent()
            dummy, data = sstruct.unpack2(
                ebdtComponentFormat, data, curComponent
            )
