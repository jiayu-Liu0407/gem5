...
class Silf(object):
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        for i in range(self.numPasses):
            ...
            p.decompile(
                data[
                    self.oPasses[i] - currpos : self.oPasses[i + 1] - currpos
                ],
                ttFont,
                version,
            )
    ...
    def compile(self, ttFont, version=2.0):
        ...
        if self.numCritFeatures:
            data += struct.pack(
                (">%dH" % self.numCritFeaturs), *self.critFeatures
            )
...
class Classes(object):
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        for s, e in zip(
            oClasses[: self.numLinear], oClasses[1 : self.numLinear + 1]
        ):
            self.linear.append(
        ...
        for s, e in zip(
            ...
            nonLinids = [
                struct.unpack(">HH", data[x : x + 4])
                for x in range(s + 8, e, 4)
            ]
            nonLin = dict(
                [(ttFont.getGlyphName(x[0]), x[1]) for x in nonLinids]
            )
    ...
    def compile(self, ttFont, version=2.0):
        ...
        return (
            ...
            + struct.pack(
                ((">%dL" if version >= 4.0 else ">%dH") % len(oClasses)),
                *oClasses
            )
            + data
        )
...
class Pass(object):
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        (self.minRulePreContext, self.maxRulePreContext) = struct.unpack(
            "BB", data[:2]
        )
        ...
        oActions = list(
            struct.unpack(
                (">%dH" % (self.numRules + 1)), data[: 2 + self.numRules * 2]
            )
        )
    ...
    def compile(self, ttFont, base, version=2.0):
        ...
        passRanges = []
        gidcolmap = dict(
            [(ttFont.getGlyphID(x[0]), x[1]) for x in self.colMap.items()]
        )
        ...
        self.pcCode = (
            self.fsmOffset
            + 2 * self.numTransitional * self.numColumns
            + 1
            + base
        )
        ...
        data += struct.pack(
            "BB", self.minRulePreContext, self.maxRulePreContext
        )
        data += struct.pack(
            (">%dH" % len(self.startStates)), *self.startStates
        )
        data += struct.pack((">%dH" % self.numRules), *self.ruleSortKeys)
        data += struct.pack(("%dB" % self.numRules), *self.rulePreContexts)
        data += struct.pack(
            ">BH", self.collisionThreshold, len(self.passConstraints)
        )
