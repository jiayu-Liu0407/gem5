...
def decompress(data):
    ...
    if scheme == 0:
        ...
        warnings.warn(
            "Table is compressed with an unsupported compression scheme"
        )
    return (data, scheme)


def compress(scheme, data):
    hdr = data[:4] + struct.pack(
        ">L", (scheme << 27) + (len(data) & 0x07FFFFFF)
    )
    if scheme == 0:
        return data
    elif scheme == 1 and lz4:
        ...
        warnings.warn(
            "Table failed to compress by unsupported compression scheme"
        )
    return data
...
def num2tag(n):
    if n < 0x200000:
        ...
        return (
            struct.unpack("4s", struct.pack(">L", n))[0]
            .replace(b"\000", b"")
            .decode()
        )
