...
class table_E_B_L_C_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        for curStrike in self.strikes:
            ...
            for subtableIndex in range(curTable.numberOfIndexSubTables):
                ...
                indexSubTable = indexFormatClass(
                    data[i + indexSubHeaderSize :], ttFont
                )
    ...
    def compile(self, ttFont):
        ...
        for _ in self.strikes:
            ...
            dataSize += len(("hori", "vert")) * sstruct.calcsize(
                sbitLineMetricsFormat
            )
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if name == "header":
            ...
            assert (
                self.strikes[strikeIndex] is None
            ), "Duplicate strike EBLC indices."
...
class BitmapSizeTable(object):
    ...
    def _getXMLMetricNames(self):
        ...
        dataNames = {
            **dataNames,
            **sstruct.getformat(bitmapSizeTableFormatPart2)[1],
        }
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for element in content:
            ...
            if name == "sbitLineMetrics":
                ...
                log.warning(
                    "unknown name '%s' being ignored in BitmapSizeTable.", name
                )
...
class EblcIndexSubTable(object):
    ...
    def removeSkipGlyphs(self):
        ...
        dataPairs = list(
            filter(isValidLocation, zip(self.names, self.locations))
        )
...
def _createOffsetArrayIndexSubTableMixin(formatStringForDataType):
    ...
    class OffsetArrayIndexSubTableMixin(object):
        def decompile(self):
            ...
            indexingOffsets = [
                glyphIndex * offsetDataSize
                for glyphIndex in range(numGlyphs + 2)
            ]
            ...
            modifiedOffsets = [
                offset + self.imageDataOffset for offset in offsetArray
            ]
        ...
        def compile(self, ttFont):
            ...
            assert all(
                glyphIds[i] < glyphIds[i + 1] for i in range(len(glyphIds) - 1)
            )
            ...
            offsets = list(allLocations[0]) + [
                loc[1] for loc in allLocations[1:]
            ]
            ...
            dataList += [
                struct.pack(dataFormat, offsetValue)
                for offsetValue in offsetArray
            ]
    ...
    def readMetrics(self, name, attrs, content, ttFont):
        for element in content:
            ...
            if name == "imageSize":
                ...
                log.warning(
                    "SmallGlyphMetrics being ignored in format %d.",
                    self.indexFormat,
                )
...
class eblc_index_sub_table_4(EblcIndexSubTable):
    ...
    def compile(self, ttFont):
        ...
        offsets = list(self.locations[0]) + [
            loc[1] for loc in self.locations[1:]
        ]
        # Image data offset must be less than or equal to the minimum of locations.
