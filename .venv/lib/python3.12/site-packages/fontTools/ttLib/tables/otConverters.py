...
class GlyphID(SimpleValue):
    ...
    def read(self, reader, font, tableDict):
        return font.getGlyphName(
            reader.readValue(self.typecode, self.staticSize)
        )
...
class Char64(SimpleValue):
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        ...
        if len(data) > self.staticSize:
            log.warning(
                'truncating overlong "%s" to %d bytes'
...
class StructWithLength(Struct):
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        ...
        assert (
            writer.items[lengthIndex] == b"\xde\xad\xbe\xef"[: conv.staticSize]
        )
...
class FeatureParams(Table):
    def getConverter(self, featureTag):
        tableClass = self.featureParamTypes.get(
            featureTag, self.defaultFeatureParams
        )
        return self.__class__(self.name, self.repeat, self.aux, tableClass)
...
class AATLookup(BaseConverter):
    ...
    def __init__(self, name, repeat, aux, tableClass, *, description=""):
        ...
        if issubclass(self.tableClass, SimpleValue):
            self.converter = self.tableClass(
                name="Value", repeat=None, aux=None
            )
        else:
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        values = list(
            sorted(
                [(font.getGlyphID(glyph), val) for glyph, val in value.items()]
            )
        )
    ...
    def readFormat4(self, reader, font):
        ...
        for i in range(reader.readUShort()):
            ...
            if last != 0xFFFF:
                dataReader = reader.getSubReader(
                    0
    ...
    def readFormat8(self, reader, font):
        ...
        return {
            font.getGlyphName(first + k): value
    ...
    def xmlRead(self, attrs, content, font):
        ...
        for element in content:
            if isinstance(element, tuple):
                ...
                if name == "Lookup":
                    value[a["glyph"]] = self.converter.xmlRead(
                        a, eltContent, font
                    )
        return value
...
class MorxSubtableConverter(BaseConverter):
    ...
    def read(self, reader, font, tableDict):
        ...
        data = reader.data[
            reader.pos : reader.pos + m.StructLength - headerLength
        ]
    ...
    def xmlWrite(self, xmlWriter, font, value, name, attrs):
        ...
        xmlWriter.newline()
        xmlWriter.simpletag(
            "SubFeatureFlags", value="0x%08x" % value.SubFeatureFlags
        )
        xmlWriter.newline()
...
class STXHeader(BaseConverter):
    def __init__(self, name, repeat, aux, tableClass, *, description=""):
        ...
        if issubclass(self.tableClass, ContextualMorphAction):
            self.perGlyphLookup = AATLookup(
                "PerGlyphLookup", None, None, GlyphID
            )
        else:
    ...
    def read(self, reader, font, tableDict):
        ...
        if issubclass(self.tableClass, LigatureMorphAction):
            ...
            numLigComponents = (
                ligaturesReader.pos - ligComponentReader.pos
            ) // 2
            assert numLigComponents >= 0
            table.LigComponents = ligComponentReader.readUShortArray(
                numLigComponents
            )
            ...
            actionReader = reader.getSubReader(0)
            actionReader.seek(pos + reader.readULong())
        table.GlyphClasses = self.classLookup.read(
            classTableReader, font, tableDict
        )
        numStates = int(
            (entryTableReader.pos - stateArrayReader.pos)
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        ...
        actionData, actionIndex = self.tableClass.compileActions(
            font, value.States
        )
    ...
    def xmlRead(self, attrs, content, font):
        ...
        for eltName, eltAttrs, eltContent in filter(istuple, content):
            if eltName == "GlyphClass":
                ...
                table.Ligatures = self._xmlReadLigatures(
                    eltAttrs, eltContent, font
                )
    ...
    def write(self, writer, font, tableDict, values, repeatIndex=None):
        ...
        return bytes(
            TupleVariation.compileDeltaValues_(
                values, optimizeSize=not optimizeSpeed
            )
        )
    ...
    def write(self, writer, font, tableDict, values, repeatIndex=None):
        ...
        offSize = (
            ...
            if lastOffset < 0x100
                ...
                else 3 if lastOffset < 0x1000000 else 4
            )
        )
...
converterMapping = {
    ...
    "AATLookupWithDataOffset": lambda C: partial(
        AATLookupWithDataOffset, tableClass=C
    ),
