...
class table__g_v_a_r(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        GVAR_HEADER_SIZE = (
            GVAR_HEADER_SIZE_HEAD + self.gid_size + GVAR_HEADER_SIZE_TAIL
        )
        header = {}
    ...
    def compileGlyphs_(self, ttFont, axisTags, sharedCoordIndices):
        ...
        for glyphName in ttFont.getGlyphOrder():
            ...
            if not variations:
                ...
                continue
            pointCountUnused = (
                0  # pointCount is actually unused by compileGlyph
            )
            result.append(
                compileGlyph_(
                    self.gid_size,
    ...
    def decompile(self, data, ttFont):
        ...
        sstruct.unpack(
            GVAR_HEADER_FORMAT_HEAD, data[:GVAR_HEADER_SIZE_HEAD], self
        )
        self.glyphCount = int.from_bytes(
            data[
                GVAR_HEADER_SIZE_HEAD : GVAR_HEADER_SIZE_HEAD + self.gid_size
            ],
            "big",
        )
        sstruct.unpack(
        ...
        def get_read_item():
            ...
            def read_item(glyphName):
                ...
                gvarData = data[
                    offsetToData + offsets[0] : offsetToData + offsets[1]
                ]
                if not gvarData:
                    return []
        ...
        l = LazyDict(
            {glyphs[gid]: read_item for gid in range(self.glyphCount)}
        )
                    ...
                    if name == "tuple":
                        ...
                        for tupleElement in content:
                            if isinstance(tupleElement, tuple):
                                ...
                                gvar.fromXML(
                                    tupleName, tupleAttrs, tupleContent
                                )
