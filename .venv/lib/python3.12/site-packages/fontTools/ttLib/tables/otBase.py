        ...
        while True:
            try:
                ...
                if state == RepackerState.HB_FT:
                    return self.tryPackingHarfbuzz(
                        writer, hb_first_error_logged
                    )
...
class OTTableReader(object):
    ...
    def copy(self):
        other = self.__class__(
            self.data, self.localState, self.offset, self.tableTag
        )
        other.pos = self.pos
        return other
    ...
    def getSubReader(self, offset):
        ...
        return self.__class__(
            self.data, self.localState, offset, self.tableTag
        )
...
class OTTableWriter(object):
    ...
    def _gatherGraphForHarfbuzz(
        self, tables, obj_list, done, objidx, virtual_edges
    ):
        real_links = []
        virtual_links = []
        item_idx = objidx
    ...
    def getAllDataUsingHarfbuzz(self, tableTag):
        ...
        objidx = 0
        virtual_edges = []
        self._gatherGraphForHarfbuzz(
            tables, obj_list, done, objidx, virtual_edges
        )
...
class BaseTable(object):
        ...
        for conv in self.getConverters():
            ...
            if conv.name == "ExtSubTable":
                conv = conv.getConverter(
                    reader.tableTag, table["ExtensionLookupType"]
                )
            ...
            try:
                if conv.repeat:
                    ...
                    countValue += conv.aux
                    table[conv.name] = conv.readArray(
                        reader, font, table, countValue
                    )
                else:
            ...
            if conv.repeat:
                ...
                if isinstance(value, CountReference):
                    ...
                    ref = writer.writeCountReference(
                        table, conv.name, conv.staticSize
                    )
        ...
        for conv in self.getConverters():
            if conv.repeat:
                ...
                for i in range(len(value)):
                    item = value[i]
                    conv.xmlWrite(
                        xmlWriter, font, item, conv.name, [("index", i)]
                    )
            else:
    ...
    class SubTableEntry(NamedTuple):
        ...
        index: Optional[
            int
        ] = None  # index into given array, None for single values
...
def getVariableAttrs(cls: BaseTable, fmt: Optional[int] = None) -> Tuple[str]:
    ...
    if issubclass(cls, FormatSwitchingBaseTable):
        if fmt is None:
            raise TypeError(
                f"'fmt' is required for format-switching {cls.__name__}"
            )
