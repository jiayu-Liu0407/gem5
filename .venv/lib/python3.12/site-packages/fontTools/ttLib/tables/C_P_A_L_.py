...
class table_C_P_A_L_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(numPalettes):
            ...
            for j in range(self.numPaletteEntries):
                palette.append(
                    Color(*struct.unpack(">BBBB", data[ppos : ppos + 4]))
                )
                ppos += 4
    ...
    def _compilePaletteLabels(self):
        if self.version == 0 or all(
            l == self.NO_NAME_ID for l in self.paletteLabels
        ):
            return b""
        assert len(self.paletteLabels) == len(self.palettes)
        result = bytesjoin(
            [struct.pack(">H", label) for label in self.paletteLabels]
        )
    ...
    def toXML(self, writer, ttFont):
        ...
        paletteLabels = {
            i: nameID for (i, nameID) in enumerate(self.paletteLabels)
        }
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if name == "palette":
            ...
            self.paletteTypes.append(
                int(attrs.get("type", self.DEFAULT_PALETTE_TYPE))
            )
            palette = []
