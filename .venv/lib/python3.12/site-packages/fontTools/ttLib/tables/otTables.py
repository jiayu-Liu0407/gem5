...
VAR_TRANSFORM_MAPPING = {
    ...
    "rotation": VarTransformMappingValues(
        VarComponentFlags.HAVE_ROTATION, 12, 180, 0
    ),
    "scaleX": VarTransformMappingValues(
        VarComponentFlags.HAVE_SCALE_X, 10, 1, 1
    ),
    "scaleY": VarTransformMappingValues(
        VarComponentFlags.HAVE_SCALE_Y, 10, 1, 1
    ),
    "skewX": VarTransformMappingValues(
        VarComponentFlags.HAVE_SKEW_X, 12, -180, 0
    ),
    "skewY": VarTransformMappingValues(
        VarComponentFlags.HAVE_SKEW_Y, 12, 180, 0
    ),
    "tCenterX": VarTransformMappingValues(
        VarComponentFlags.HAVE_TCENTER_X, 0, 1, 0
    ),
    "tCenterY": VarTransformMappingValues(
...
def _read_uint32var(data, i):
    ...
    if b0 < 0x80:
        ...
        return (b0 - 0xF0) << 32 | data[i + 1] << 24 | data[
            i + 2
        ] << 16 | data[i + 3] << 8 | data[i + 4], i + 5
...
class VarComponent:
    ...
    def decompile(self, data, font, localState):
        ...
        if self.axisIndicesIndex is None:
            ...
            axisIndices = localState["AxisIndicesList"].Item[
    ...
    def toXML(self, writer, ttFont, attrs):
        ...
        if (
            ...
            write(
                "axisValues",
                [float(fl2str(v, 14)) for v in self.axisValues],
                attrs,
            )
    ...
    def applyTransformDeltas(self, deltas):
        ...
        for attr_name, mapping_values in VAR_TRANSFORM_MAPPING.items():
            ...
            setattr(
                self.transform,
                attr_name,
                getattr(self.transform, attr_name) + value,
            )
...
class AATAction(object):
    ...
    def _writeFlagsToXML(self, xmlWriter):
        ...
        if self.ReservedFlags != 0:
            xmlWriter.simpletag(
                "ReservedFlags", value="0x%04X" % self.ReservedFlags
            )
            xmlWriter.newline()
...
class FeatureParams(BaseTable):
    ...
    def toXML(self, xmlWriter, font, attrs=None, name=None):
        BaseTable.toXML(
            self, xmlWriter, font, attrs, name=self.__class__.__name__
        )
...
class Coverage(FormatSwitchingBaseTable):
    ...
    def preWrite(self, font):
        ...
        if glyphs:
            ...
            if brokenOrder or len(ranges) * 3 < len(
                glyphs
            ):  # 3 words vs. 1 word
...
class VarRegionList(BaseTable):
    def preWrite(self, font):
        ...
        return {
            **self.__dict__,
            "RegionAxisCount": CountReference(
                self.__dict__, "RegionAxisCount"
            ),
        }
...
class SingleSubst(FormatSwitchingBaseTable):
    ...
    def toXML2(self, xmlWriter, font):
        ...
        for inGlyph, outGlyph in items:
            xmlWriter.simpletag(
                "Substitution", [("in", inGlyph), ("out", outGlyph)]
            )
            xmlWriter.newline()
...
class MultipleSubst(FormatSwitchingBaseTable):
    ...
    def preWrite(self, font):
        ...
        rawTable = {
            "Coverage": cov,
            "Sequence": [
                self.makeSequence_(mapping[glyph]) for glyph in cov.glyphs
            ],
        }
        return rawTable
    ...
    def toXML2(self, xmlWriter, font):
        ...
        for inGlyph, outGlyphs in items:
            out = ",".join(outGlyphs)
            xmlWriter.simpletag(
                "Substitution", [("in", inGlyph), ("out", out)]
            )
            xmlWriter.newline()
...
class ClassDef(FormatSwitchingBaseTable):
    ...
    def toXML2(self, xmlWriter, font):
        ...
        for glyphName, cls in items:
            xmlWriter.simpletag(
                "ClassDef", [("glyph", glyphName), ("class", cls)]
            )
            xmlWriter.newline()
...
class LigatureSubst(FormatSwitchingBaseTable):
    ...
    def preWrite(self, font):
        ...
        if ligatures and isinstance(next(iter(ligatures)), tuple):
            ...
            newLigatures = dict()
            for comps in sorted(
                ligatures.keys(), key=self._getLigatureSortKey
            ):
                ligature = Ligature()
...
class COLR(BaseTable):
    ...
    def preWrite(self, font):
        ...
        return {
            **self.__dict__,
            "LayerRecordCount": CountReference(
                self.__dict__, "LayerRecordCount"
            ),
        }
    ...
    def computeClipBoxes(self, glyphSet: "_TTGlyphSet", quantization: int = 1):
        ...
        for rec in self.BaseGlyphList.BaseGlyphPaintRecord:
            try:
                clipBox = rec.Paint.computeClipBox(
                    self, glyphSet, quantization
                )
...
class Paint(getFormatSwitchingBaseTableClass("uint8")):
    ...
    def iterPaintSubTables(
        self, colr: COLR
    ) -> Iterator[BaseTable.SubTableEntry]:
        if self.Format == PaintFormat.PaintColrLayers:
            ...
            yield from (
                ...
                for i, v in enumerate(
                    layers[
                        self.FirstLayerIndex : self.FirstLayerIndex
                        + self.NumLayers
                    ]
                )
            )
        ...
        if self.Format == PaintFormat.PaintColrGlyph:
            for record in colr.BaseGlyphList.BaseGlyphPaintRecord:
                if record.BaseGlyph == self.Glyph:
                    yield BaseTable.SubTableEntry(
                        name="BaseGlyph", value=record.Paint
                    )
                    return
            else:
        ...
        for conv in self.getConverters():
            if conv.tableClass is not None and issubclass(
                conv.tableClass, type(self)
            ):
                value = getattr(self, conv.name)
    ...
    def traverse(self, colr: COLR, callback):
        ...
        for path in dfs_base_table(
            self,
            iter_subtables_fn=lambda paint: paint.iterPaintSubTables(colr),
        ):
            paint = path[-1].value
            callback(paint)
    ...
    def getTransform(self) -> Transform:
        ...
        if PaintFormat(self.Format).is_variable():
            raise NotImplementedError(
                f"Variable Paints not supported: {self.Format}"
            )
    ...
    def computeClipBox(
        ...
        cb.xMin, cb.yMin, cb.xMax, cb.yMax = quantizeRect(
            pen.bounds, quantization
        )
        return cb
...
def splitPairPos(oldSubTable, newSubTable, overflowRecord):
    ...
    if oldSubTable.Format == 1 and len(oldSubTable.PairSet) > 1:
        ...
        if not hasattr(oldSubTable, "Class2Count"):
            oldSubTable.Class2Count = len(
                oldSubTable.Class1Record[0].Class2Record
            )
        ...
        oldSubTable.Coverage.glyphs = [
            g for g in coverage if g not in newGlyphs
        ]
...
def _buildClasses():
    ...
    for name, table in otData:
        ...
        if m:
            ...
            converters, convertersByName = buildConverters(
                table[1:], namespace
            )
            ...
            cls = namespace[name]
            cls.converters, cls.convertersByName = buildConverters(
                table, namespace
            )
            # XXX Add staticSize?
