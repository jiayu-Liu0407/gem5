    ...
    def getcmap(self, platformID, platEncID):
        ...
        for subtable in self.tables:
            if (
                ...
                and subtable.platEncID == platEncID
            ):
                return subtable
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(numSubTables):
            ...
            if format in [8, 10, 12, 13]:
                ...
                format, length = struct.unpack(
                    ">HL", data[offset : offset + 6]
                )
    ...
    def compile(self, ttFont):
        ...
        for table in self.tables:
            ...
            if offset is None:
                ...
                if offset is None:
                    offset = seen[id(table.cmap)] = done[
                        chunk
                    ] = totalOffset + len(tableData)
                    tableData = tableData + chunk
            data = data + struct.pack(
                ">HHl", table.platformID, table.platEncID, offset
            )
...
class CmapSubtable(object):
    ...
    def ensureDecompiled(self, recurse=False):
        ...
        self.data = (
            None  # Once this table has been decompiled, make sure we don't
        )
    ...
    def __getattr__(self, attr):
        ...
        if (
            attr[:2] == "__"
        ):  # don't handle requests for member functions like '__lt__'
    ...
    def getEncoding(self, default=None):
        ...
        """
        return getEncoding(
            self.platformID, self.platEncID, self.language, default
        )
...
class cmap_format_0(CmapSubtable):
    ...
    def compile(self, ttFont):
        ...
        valueList = [
            getGlyphID(cmap[i]) if i in cmap else 0 for i in range(256)
        ]
...
class cmap_format_2(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(maxSubHeaderindex + 1):
            ...
            giList.frombytes(
                data[giDataPos : giDataPos + subHeader.entryCount * 2]
            )
            if sys.byteorder != "big":
                giList.byteswap()
    ...
    def compile(self, ttFont):
        if self.data:
            return (
                struct.pack(">HHH", self.format, self.length, self.language)
                + self.data
            )
...
class cmap_format_4(CmapSubtable):
    def decompile(self, data, ttFont):
        ...
        allCodes = allCodes[
            segCount + 1 :
        ]  # the +1 is skipping the reservedPad field
    ...
    def compile(self, ttFont):
        ...
        for i in range(len(endCode) - 1):  # skip the closing codes (0xffff)
            ...
            if indices == list(range(indices[0], indices[0] + len(indices))):
                ...
                idDelta.append(0)
                idRangeOffset.append(
                    2 * (len(endCode) + len(glyphIndexArray) - i)
                )
                glyphIndexArray.extend(indices)
        idDelta.append(
            1
        )  # 0xffff + 1 == (tadaa!) 0. So this end code maps to .notdef
        idRangeOffset.append(0)
        ...
        if sys.byteorder != "big":
            restArray.byteswap()
        data = (
            charCodeArray.tobytes()
            + idDeltaArray.tobytes()
            + restArray.tobytes()
        )
...
class cmap_format_12_or_13(CmapSubtable):
    ...
    def compile(self, ttFont):
        ...
        for index in range(maxIndex):
            ...
            if not self._IsInSameRun(
                glyphID, lastGlyphID, charCode, lastCharCode
            ):
                dataList.append(
                    struct.pack(
                        ">LLL", startCharCode, lastCharCode, startGlyphID
                    )
                )
...
class cmap_format_14(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        for n in range(self.numVarSelectorRecords):
            ...
            if defOVSOffset:
                ...
                (numValues,) = struct.unpack(
                    ">L", data[startOffset : startOffset + 4]
                )
                startOffset += 4
            ...
            if nonDefUVSOffset:
                ...
                (numRecs,) = struct.unpack(
                    ">L", data[startOffset : startOffset + 4]
                )
                startOffset += 4
                localUVList = []
                for r in range(numRecs):
                    uv, gid = struct.unpack(
                        ">3sH", data[startOffset : startOffset + 5]
                    )
                    startOffset += 5
                    uv = cvtToUVS(uv)
    ...
    def toXML(self, writer, ttFont):
        ...
        for uvs in uvsList:
            uvList = uvsDict[uvs]
            uvList.sort(
                key=lambda item: (item[1] is not None, item[0], item[1])
            )
    ...
    def fromXML(self, name, attrs, content, ttFont):
        self.language = (
            0xFF  # provide a value so that CmapSubtable.__lt__() won't fail
        )
        if not hasattr(self, "cmap"):
            self.cmap = (
                {}
    ...
    def compile(self, ttFont):
        ...
        for uvs in uvsList:
            ...
            vrec = struct.pack(
                ">3sLL", cvtFromUVS(uvs), defOVSOffset, nonDefUVSOffset
            )
