...
class WOFF2Writer(SFNTWriter):
    ...
    def _calcSFNTChecksumsLengthsAndOffsets(self):
        ...
        for tag, entry in self.tables.items():
            ...
            if tag == "head":
                entry.checkSum = calcChecksum(
                    data[:8] + b"\0\0\0\0" + data[12:]
                )
            else:
    ...
    def _calcMasterChecksum(self):
        ...
        directory_end = (
            sfntDirectorySize + len(self.tables) * sfntDirectoryEntrySize
        )
...
class WOFF2GlyfTable(getTableClass("glyf")):
    ...
    def reconstruct(self, data, ttFont):
        ...
        if hasOverlapSimpleBitmap:
            ...
            self.overlapSimpleBitmap = array.array(
                "B", data[:overlapSimpleBitmapSize]
            )
    ...
    def _decodeBBox(self, glyphID, glyph):
        ...
        if haveBBox:
            dummy, self.bboxStream = sstruct.unpack2(
                bboxFormat, self.bboxStream, glyph
            )
        else:
    ...
    def _decodeTriplets(self, glyph):
        ...
        for i in range(nPoints):
            ...
            if flag < 10:
                dx = 0
                dy = withSign(
                    flag, ((flag & 14) << 7) + triplets[tripletIndex]
                )
            elif flag < 20:
                dx = withSign(
                    flag, (((flag - 10) & 14) << 7) + triplets[tripletIndex]
                )
                dy = 0
            elif flag < 84:
                b0 = flag - 20
                ...
                b0 = flag - 84
                dx = withSign(
                    flag, 1 + ((b0 // 12) << 8) + triplets[tripletIndex]
                )
                dy = withSign(
                    flag >> 1,
    ...
    def _encodeComponents(self, glyph):
        ...
        for i in range(len(glyph.components)):
            ...
            self.compositeStream += component.compile(
                more, haveInstructions, self
            )
        if haveInstructions:
    ...
    def _encodeTriplets(self, glyph):
        ...
        for i in range(len(coordinates)):
            ...
            if x == 0 and absY < 1280:
                ...
                flags.append(
                    onCurveBit + 10 + ((absX & 0xF00) >> 7) + xSignBit
                )
...
class WOFF2HmtxTable(getTableClass("hmtx")):
    ...
    def reconstruct(self, data, ttFont):
        ...
        if flags & 0b11111100 != 0:
            raise TTLibError(
                "Bits 2-7 of '%s' flags are reserved" % self.tableTag
            )
...
class WOFF2FlavorData(WOFFFlavorData):
    ...
    def __init__(self, reader=None, data=None, transformedTables=None):
        ...
        if reader:
            ...
            if transformedTables is None and hasattr(
