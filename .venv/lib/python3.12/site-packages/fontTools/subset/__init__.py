...
def _log_glyphs(self, glyphs, font=None):
    ...
    if font:
        ...
        self.info(
            "Glyph IDs:   %s", sorted(reverseGlyphMap[g] for g in glyphs)
        )
...
def intersect_class(self, glyphs, klass):
    ...
    return set(
        g for g, v in self.classDefs.items() if v == klass and g in glyphs
    )
...
def subset(self, glyphs, remap=False, useClass0=True):
    ...
    indices = _uniq_sort(
        (
            [0]
            if (
                (not useClass0) or any(g not in self.classDefs for g in glyphs)
            )
            else []
        )
...
def closure_glyphs(self, s, cur_glyphs):
    s.glyphs.update(
        *(vlist for g, vlist in self.alternates.items() if g in cur_glyphs)
    )
...
def subset_glyphs(self, s):
    ...
    self.ligatures = {
        g: [
            seq
            for seq in seqs
            if seq.LigGlyph in s.glyphs
            and all(c in s.glyphs for c in seq.Component)
        ]
        for g, seqs in self.ligatures.items()
    }
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        indices = [
            i for i, sub in enumerate(self.Substitute) if sub in s.glyphs
        ]
        self.Substitute = _list_subset(self.Substitute, indices)
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class2_map = [
            c
            for c in self.ClassDef2.subset(
                s.glyphs, remap=True, useClass0=False
            )
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        self.LigatureArray.LigatureCount = len(
            self.LigatureArray.LigatureAttach
        )
        # Prune empty classes
        class_indices = _uniq_sort(v.Class for v in self.MarkArray.MarkRecord)
        self.ClassCount = len(class_indices)
        ...
        for l in self.LigatureArray.LigatureAttach:
            ...
            for c in l.ComponentRecord:
                c.LigatureAnchor = _list_subset(
                    c.LigatureAnchor, class_indices
                )
        return bool(
            self.ClassCount
            and self.MarkArray.MarkCount
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        return bool(
            self.ClassCount
            and self.Mark1Array.MarkCount
            and self.Mark2Array.MarkCount
        )
    else:
        assert 0, "unknown format: %s" % self.Format
...
def closure_glyphs(self, s, cur_glyphs):
    ...
    if self.Format == 1:
        ...
        if not all(
            x is not None and x.intersect(s.glyphs) for x in c.RuleData(self)
        ):
            return []
        r = self
    ...
    if self.Format == 1:
        ...
        for rs in rss:
            ...
            ss = [
                ...
                if r
                and all(
                    all(g in s.glyphs for g in glist)
        ...
        return all(
            x is not None and x.subset(s.glyphs) for x in c.RuleData(self)
        )
    else:
        assert 0, "unknown format: %s" % self.Format
        ...
        for rs in getattr(self, c.RuleSet):
            ...
            for r in getattr(rs, c.Rule):
                ...
                for ll in getattr(r, c.LookupRecord):
                    if not ll:
                        continue
                    ll.LookupListIndex = lookup_indices.index(
                        ll.LookupListIndex
                    )
    elif self.Format == 3:
        setattr(
...
def collect_lookups(self):
    ...
    if self.Format in [1, 2]:
        ...
        return [
            ll.LookupListIndex for ll in getattr(self, c.LookupRecord) if ll
        ]
    else:
        assert 0, "unknown format: %s" % self.Format
...
def subset_glyphs(self, s):
    ...
    if hasattr(self, "MarkFilteringSet") and self.MarkFilteringSet is not None:
        if self.MarkFilteringSet not in s.used_mark_sets:
            ...
            self.MarkFilteringSet = s.used_mark_sets.index(
                self.MarkFilteringSet
            )
...
def subset_lookups(self, lookup_indices):
    ...
    self.Lookup = [
        self.Lookup[i] for i in lookup_indices if i < self.LookupCount
    ]
...
def subset_lookups(self, lookup_indices):
    ...
    self.LookupListIndex = [
        l for l in self.LookupListIndex if l in lookup_indices
    ]
    # Now map them.
    self.LookupListIndex = [
        lookup_indices.index(l) for l in self.LookupListIndex
    ]
...
def subset_features(self, feature_indices):
    ...
    # Now map them.
    self.FeatureIndex = [
        feature_indices.index(f)
        for f in self.FeatureIndex
...
def collect_features(self):
    feature_indices = [
        l.LangSys.collect_features() for l in self.LangSysRecord
    ]
...
def collect_features(self):
    return _uniq_sort(
        sum((s.Script.collect_features() for s in self.ScriptRecord), [])
    )
...
def subset_glyphs(self, s):
    for strike in self.strikes:
        for indexSubTable in strike.indexSubTables:
            indexSubTable.names = [
                n for n in indexSubTable.names if n in s.glyphs
            ]
...
def subset_glyphs(self, s):
    for strike in self.strikes.values():
        strike.glyphs = {
            g: strike.glyphs[g] for g in s.glyphs if g in strike.glyphs
        }
...
def closure_glyphs(self, s):
    ...
    if self.table.FeatureList:
        lookup_indices = self.table.FeatureList.collect_lookups(
            feature_indices
        )
    else:
        lookup_indices = []
    if getattr(self.table, "FeatureVariations", None):
        lookup_indices += self.table.FeatureVariations.collect_lookups(
            feature_indices
        )
    lookup_indices = _uniq_sort(lookup_indices)
...
def subset_lookups(self, lookup_indices):
    ...
    if self.table.FeatureList:
        ...
        feature_indices = []
    if getattr(self.table, "FeatureVariations", None):
        feature_indices += self.table.FeatureVariations.subset_lookups(
            lookup_indices
        )
...
def subset_script_tags(self, tags):
    ...
    if self.table.ScriptList:
        self.table.ScriptList.ScriptRecord = [
            s
            for s in self.table.ScriptList.ScriptRecord
            if s.ScriptTag in script_tags
        ]
        self.table.ScriptList.ScriptCount = len(
            self.table.ScriptList.ScriptRecord
        )

        for record in self.table.ScriptList.ScriptRecord:
            if (
                record.ScriptTag in langsys
                and "*   " not in langsys[record.ScriptTag]
            ):
...
def prune_post_subset(self, font, options):
    ...
    if hasattr(table, "FeatureVariations"):
        ...
        if table.FeatureVariations and not (
            table.FeatureList
            and table.FeatureVariations.FeatureVariationRecord
        ):
...
def subset_glyphs(self, s):
    ...
    if table.LigCaretList:
        ...
        table.LigCaretList.LigGlyph = _list_subset(
            table.LigCaretList.LigGlyph, indices
        )
    ...
    if table.MarkAttachClassDef:
        table.MarkAttachClassDef.classDefs = {
            g: v
            for g, v in table.MarkAttachClassDef.classDefs.items()
            if g in glyphs
        }
    if table.GlyphClassDef:
        table.GlyphClassDef.classDefs = {
            g: v
            for g, v in table.GlyphClassDef.classDefs.items()
            if g in glyphs
        }
    if table.AttachList:
    ...
    if hasattr(table, "MarkGlyphSetsDef") and table.MarkGlyphSetsDef:
        ...
        s.used_mark_sets = [
            i for i, c in enumerate(markGlyphSets.Coverage) if c.glyphs
        ]
        markGlyphSets.Coverage = [
            c for c in markGlyphSets.Coverage if c.glyphs
        ]
...
def subset_glyphs(self, s):
    ...
    if table.Format in (1, 3):
        ...
        if len(baselines) > 0:
            ...
            baselines = {
                glyph: b for glyph, b in baselines.items() if b != mostCommon
            }
        if len(baselines) > 0:
...
def subset_glyphs(self, s):
    ...
    if table.AdvWidthMap:
        table.AdvWidthMap.mapping = _dict_subset(
            table.AdvWidthMap.mapping, s.glyphs
        )
    ...
    if table.AdvWidthMap:
        table.AdvWidthMap.mapping = _remap_index_map(
            s, varidx_map, table.AdvWidthMap
        )
    if table.LsbMap:
...
def subset_glyphs(self, s):
    ...
    if table.AdvHeightMap:
        table.AdvHeightMap.mapping = _dict_subset(
            table.AdvHeightMap.mapping, s.glyphs
        )
    ...
    if table.AdvHeightMap:
        table.AdvHeightMap.mapping = _remap_index_map(
            s, varidx_map, table.AdvHeightMap
        )
    if table.TsbMap:
...
def subset_glyphs(self, s):
    ...
    if prop.Format == 0:
        ...
        prop.Properties = {
            g: prop
            for g, prop in prop.Properties.items()
            if prop != mostCommon
        }
...
def subset_glyphs(self, s):
    ...
    colorGlyphsV1 = unbuildColrV1(
        self.table.LayerList, self.table.BaseGlyphList
    )
    ...
    if self.table.ClipList is not None:
        ...
        self.table.ClipList.clips = {
            g: clips[g] for g in clips if g in s.glyphs
        }
...
def prune_post_subset(self, font, options):
    ...
    for palette in self.palettes:
        palette[:] = [
            c for i, c in enumerate(palette) if i in retained_palette_indices
        ]
...
def closure_glyphs(self, s):
    ...
    if self.VertGlyphCoverage:
        ...
        for i in indices:
            variants.update(
                self.VertGlyphConstruction[i].closure_glyphs(glyphs)
            )
    ...
    if self.HorizGlyphCoverage:
        ...
        for i in indices:
            variants.update(
                self.HorizGlyphConstruction[i].closure_glyphs(glyphs)
            )
...
def closure_glyphs(self, s):
    ...
    glyphMap = {
        glyphName: i for i, glyphName in enumerate(self.table.Coverage.glyphs)
    }
...
def prune_post_subset(self, font, options):
    ...
    if conditionList is not None:
        ...
        usedIndices = sorted(usedIndices)
        conditionList.ConditionTable = _list_subset(
            conditionTables, usedIndices
        )
        mapping = {old: new for new, old in enumerate(usedIndices)}
        for glyph in table.VarCompositeGlyphs.VarCompositeGlyph:
            for comp in glyph.components:
...
def subset_glyphs(self, s):
    if self.VertGlyphCoverage:
        ...
        self.VertGlyphConstruction = _list_subset(
            self.VertGlyphConstruction, indices
        )
        self.VertGlyphCount = len(self.VertGlyphConstruction)

    if self.HorizGlyphCoverage:
        indices = self.HorizGlyphCoverage.subset(s.glyphs)
        self.HorizGlyphConstruction = _list_subset(
            self.HorizGlyphConstruction, indices
        )
...
def subset_glyphs(self, s):
    ...
    table_plat0_enc3 = (
        {}
    )  # Unicode platform, Unicode BMP only, keyed by language
    ...
    for t in self.tables:
        ...
        if t.format == 14:
            ...
            t.cmap = {
                u: g for u, g in t.cmap.items() if g in s.glyphs_requested
            }
    ...
    self.tables = [
        t for t in self.tables if (t.cmap if t.format != 14 else t.uvsDict)
    ]
    self.numSubTables = len(self.tables)
...
def prune_post_subset(self, font, options):
    ...
    if "*" not in options.name_languages:
        ...
        self.names = [
            n for n in self.names if n.langID in options.name_languages
        ]
    if options.obfuscate_names:
        ...
        for n in self.names:
            if n.nameID in [1, 4]:
                n.string = (
                    ".\x7f".encode("utf_16_be") if n.isUnicode() else ".\x7f"
                )
...
def _remap_select_name_ids(
    font: ttLib.TTFont, needRemapping: set[int]
) -> None:
    ...
    for nameId in needRemapping:
        ...
        if nextId > 32767:
            raise ValueError(
                "Ran out of name IDs while trying to remap existing ones."
            )
    ...
    if stat.table.AxisValueArray:
        for value in stat.table.AxisValueArray.AxisValue:
            value.ValueNameID = remapping.get(
                value.ValueNameID, value.ValueNameID
            )
...
class Options(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.obfuscate_names = (
            False  # to make webfont unusable as a system font
        )
        self.retain_gids = False
        self.notdef_glyph = True  # gid0 for TrueType / .notdef for CFF
        self.notdef_outline = (
            False  # No need for notdef to have an outline really
        )
        ...
        self.prune_codepage_ranges = (
            True  # Clear unused 'ulCodePageRange' bits
        )
            ...
            if isinstance(ov, bool):
                ...
                if isinstance(v, bool):
                    raise self.OptionError(
                        "Option '%s' requires values to be specified using '='"
                        % a
                    )
                vv = v.replace(",", " ").split()
                if vv == [""]:
                    vv = []
                vv = [
                    int(x, 0) if len(x) and x[0] in "0123456789" else x
                    for x in vv
                ]
                if op == "=":
                    v = vv
...
class Subsetter(object):
    ...
    def _prune_pre_subset(self, font):
        for tag in self._sort_tables(font):
            if (
                ...
                or (
                    tag == "kern"
                    and (not self.options.legacy_kern and "GPOS" in font)
                )
            ):
    ...
    def _closure_glyphs(self, font):
        ...
        self.glyphs_missing.update(
            self.glyphs_requested.difference(realGlyphs)
        )
        self.glyphs_missing.update(
        ...
        if "MATH" in font:
            with timer("close glyph list over 'MATH'"):
                log.info(
                    "Closing glyph list over 'MATH': %d glyphs before",
                    len(self.glyphs),
                )
                ...
                self.glyphs.intersection_update(realGlyphs)
                log.info(
                    "Closed glyph list over 'MATH': %d glyphs after",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
        ...
        if self.options.layout_closure and "GSUB" in font:
            with timer("close glyph list over 'GSUB'"):
                log.info(
                    "Closing glyph list over 'GSUB': %d glyphs before",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
                ...
                self.glyphs.intersection_update(realGlyphs)
                log.info(
                    "Closed glyph list over 'GSUB': %d glyphs after",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
        ...
        if "VARC" in font:
            with timer("close glyph list over 'VARC'"):
                log.info(
                    "Closing glyph list over 'VARC': %d glyphs before",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
                ...
                self.glyphs.intersection_update(realGlyphs)
                log.info(
                    "Closed glyph list over 'VARC': %d glyphs after",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
        ...
        if "glyf" in font:
            with timer("close glyph list over 'glyf'"):
                log.info(
                    "Closing glyph list over 'glyf': %d glyphs before",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
                ...
                self.glyphs.intersection_update(realGlyphs)
                log.info(
                    "Closed glyph list over 'glyf': %d glyphs after",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
        self.glyphs_glyfed = frozenset(self.glyphs)
        ...
        if "CFF " in font:
            with timer("close glyph list over 'CFF '"):
                log.info(
                    "Closing glyph list over 'CFF ': %d glyphs before",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
                ...
                self.glyphs.intersection_update(realGlyphs)
                log.info(
                    "Closed glyph list over 'CFF ': %d glyphs after",
                    len(self.glyphs),
                )
                log.glyphs(self.glyphs, font=font)
        ...
        self.last_retained_glyph = font.getGlyphOrder()[
            self.last_retained_order
        ]
        ...
        self.reverseEmptiedGlyphMap = {
            g: order[g] for g in self.glyphs_emptied
        }
    ...
    def _subset_glyphs(self, font):
        ...
        for tag in self._sort_tables(font):
            ...
            if tag.strip() in self.options.no_subset_tables:
                ...
                log.warning(
                    "%s NOT subset; don't know how to subset; dropped", tag
                )
                del font[tag]
        ...
        for tag in tableTags:
            ...
            if tag == "OS/2":
                if self.options.prune_unicode_ranges:
                    ...
                    new_uniranges = font[tag].recalcUnicodeRanges(
                        font, pruneOnly=True
                    )
                    if old_uniranges != new_uniranges:
                        log.info(
                            "%s Unicode ranges pruned: %s",
                            tag,
                            sorted(new_uniranges),
                        )
                if (
                    self.options.prune_codepage_ranges
                    and font[tag].version >= 1
                ):
                    ...
                    new_codepages = font[tag].recalcCodePageRanges(
                        font, pruneOnly=True
                    )
                ...
                if self.options.recalc_average_width:
                    ...
                    if old_avg_width != new_avg_width:
                        log.info(
                            "%s xAvgCharWidth updated: %d", tag, new_avg_width
                        )
                if self.options.recalc_max_context:
                    ...
                    if max_context != font[tag].usMaxContext:
                        ...
                        log.info(
                            "%s usMaxContext updated: %d", tag, max_context
                        )
            clazz = ttLib.getTableClass(tag)
...
@timer("load font")
def load_font(
    fontFile, options, checkChecksums=0, dontLoadGlyphNames=False, lazy=True
):
    font = ttLib.TTFont(
        fontFile,
...
def main(args=None):
    ...
    if options.verbose:
        ...
        log.info(
            "Input font:% 7d bytes: %s" % (os.path.getsize(fontfile), fontfile)
        )
        log.info(
            "Subset font:% 7d bytes: %s" % (os.path.getsize(outfile), outfile)
        )
