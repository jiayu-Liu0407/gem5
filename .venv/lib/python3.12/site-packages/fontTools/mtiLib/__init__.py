...
class DeferredMapping(dict):
    ...
    def addDeferredMapping(self, setter, sym, e):
        log.debug(
            "Adding deferred mapping for symbol '%s' %s", sym, type(e).__name__
        )
    ...
    def applyDeferredMappings(self):
        for setter, sym, e in self._deferredMappings:
            log.debug(
                "Applying deferred mapping for symbol '%s' %s",
                sym,
                type(e).__name__,
            )
            try:
...
def parseScriptList(lines, featureMap=None):
    ...
    with lines.between("script table"):
        for line in lines:
            ...
            log.debug(
                "Adding script %s language-system %s", scriptTag, langSysTag
            )
            ...
            if langSysTag == "default":
                ...
                langSysRec.LangSysTag = langSysTag + " " * (
                    4 - len(langSysTag)
                )
    ...
    with lines.between("feature table"):
        for line in lines:
            ...
            if featureMap is not None:
                assert name not in featureMap, (
                    "Duplicate feature name: %s" % name
                )
...
def parsePair(lines, font, _lookupMap=None):
    ...
    if typ in ("left", "right"):
        ...
        self.Class1Record = [
            ot.Class1Record() for i in range(self.Class1Count)
        ]
        for rec1 in self.Class1Record:
            rec1.Class2Record = [
                ot.Class2Record() for j in range(self.Class2Count)
            ]
        ...
        self.Coverage = makeCoverage(
            set(self.ClassDef1.classDefs.keys()), font
        )
    else:
        assert 0, typ
    return self
    ...
    for (glyph, klass, compIdx, compCount), anchor in data.items():
        ...
        if record is None:
            ...
            record.ComponentRecord = [
                ot.ComponentRecord() for i in range(compCount)
            ]
...
def parseMarkToSomething(lines, font, c):
    ...
    # Mark
    markCoverage = makeCoverage(
        set(markData.keys()), font, c.MarkCoverageClass
    )
    ...
    # Base
    self.classCount = (
        0 if not baseData else 1 + max(k[1] for k, v in baseData.items())
    )
    ...
    if c.Base == "Ligature":
        baseRecords = makeLigatureRecords(
            baseData, baseCoverage, c, self.classCount
        )
    else:
...
def parseLookupRecords(items, klassName, lookupMap=None):
    ...
    for item in items:
        ...
        setReference(
            mapLookup, lookupMap, item[1], setattr, rec, "LookupListIndex"
        )
        lst.append(rec)
    return lst
...
def bucketizeRules(self, c, rules, bucketKeys):
    ...
    for seq, recs in rules:
        buckets.setdefault(seq[c.InputIdx][0], []).append(
            (
                tuple(
                    s[1 if i == c.InputIdx else 0 :] for i, s in enumerate(seq)
                ),
                recs,
            )
        )
    ...
    if typ == "glyph":
        ...
        for line in lines:
            ...
            while len(line) < 1 + c.DataLen:
                line.append("")
            seq = tuple(
                makeGlyphs(stripSplitComma(i)) for i in line[1 : 1 + c.DataLen]
            )
            recs = parseLookupRecords(
                line[1 + c.DataLen :], c.LookupRecord, lookupMap
            )
    ...
    while lines.peek() is not None:
        ...
        if typ == "lookup":
            ...
            if lookupMap is not None:
                assert name not in lookupMap, (
                    "Duplicate lookup name: %s" % name
                )
...
def parseGDEF(lines, font):
    ...
    fields = {
        "class definition begin": (
            "GlyphClassDef",
            lambda lines, font: parseClassDef(
                lines, font, klass=ot.GlyphClassDef
            ),
        ),
...
class Tokenizer(object):
    ...
    def _next_line(self):
        ...
        if line and not line[-1]:
            log.warning(
                "trailing tab found on line %d: %s" % (self.lineno, self.line)
            )
    ...
    def _next_nonempty(self):
        while True:
            ...
            if (
                line
                and line[0]
                and (line[0][0] != "%" or line[0] == "% subtable")
            ):
                return line
