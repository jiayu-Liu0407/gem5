...
class PointToSegmentPen(BasePointToSegmentPen):
    ...
    def __init__(
        self, segmentPen, outputImpliedClosingLine: bool = False
    ) -> None:
...
class GuessSmoothPointPen(AbstractPointPen):
    ...
    def addVarComponent(
        ...
        if self._points is not None:
            raise PenError(
                "VarComponents must be added before or after contours"
            )
        if identifier is not None:
            kwargs["identifier"] = identifier
        self._outPen.addVarComponent(
            glyphName, transformation, location, **kwargs
        )
...
class ReverseContourPointPen(AbstractPointPen):
    ...
    def addComponent(self, glyphName, transform, identifier=None, **kwargs):
        if self.currentContour is not None:
            raise PenError("Components must be added before or after contours")
        self.pen.addComponent(
            glyphName, transform, identifier=identifier, **kwargs
        )
