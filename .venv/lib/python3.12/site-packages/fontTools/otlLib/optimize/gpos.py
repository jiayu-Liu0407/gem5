...
def compact(font: TTFont, level: int) -> TTFont:
    ...
    for lookup in gpos.table.LookupList.Lookup:
        if lookup.LookupType == 2:
            ...
            lookup.LookupType == 9
            and lookup.SubTable[0].ExtensionLookupType == 2
        ):
...
def compact_ext_lookup(
    font: TTFont, level: int, lookup: otTables.Lookup
) -> None:
    new_subtables = compact_pair_pos(
        font,
        level,
        [ext_subtable.ExtSubTable for ext_subtable in lookup.SubTable],
    )
    new_ext_subtables = []
...
class Cluster:
    ...
    def coverage_bytes(self):
        ...
        ranges = sorted(
            chain.from_iterable(
                self.ctx.all_class1_data[i][0] for i in self.indices
            )
        )
    ...
    def classDef1_bytes(self):
        ...
        biggest_index = max(
            self.indices, key=lambda i: len(self.ctx.all_class1[i])
        )
        return _classDef_bytes(
            self.ctx.all_class1_data,
            [i for i in self.indices if i != biggest_index],
        )
    ...
    while len(clusters) > 1:
        ...
        for i, cluster in enumerate(clusters):
            for j, other in enumerate(clusters[i + 1 :]):
                ...
                if (
                    lowest_cost_change is None
                    or cost_change < lowest_cost_change
                ):
