...
def buildLookup(
    subtables, flags=0, markFilterSet=None, table=None, extension=False
):
...
class AlternateSubstBuilder(LookupBuilder):
    ...
    def add_subtable_break(self, location):
        self.alternates[
            (self.SUBTABLE_BREAK_, location)
        ] = self.SUBTABLE_BREAK_


class ChainContextualRule(
    namedtuple(
        "ChainContextualRule", ["prefix", "glyphs", "suffix", "lookups"]
    )
):
    @property
...
class ChainContextualBuilder(LookupBuilder):
        ...
        for ruleset in rulesets:
            ...
            if not candidates_by_size:
                raise OpenTypeLibError(
                    "All candidates overflowed", self.location
                )
    ...
    def buildFormat1Subtable(self, ruleset, chaining=True):
        ...
        setattr(
            st,
            self.ruleSetAttr_(format=1, chaining=chaining) + "Count",
            len(ruleSets),
        )
    ...
    def buildFormat2Subtable(self, ruleset, classdefs, chaining=True):
        ...
        for rule in ruleset.rules:
            ...
            if chaining:
                ...
                ruleAsSubtable.LookAhead = [
                    st.LookAheadClassDef.classDefs[list(x)[0]]
                    for x in rule.suffix
                ]

                ruleAsSubtable.InputGlyphCount = len(rule.glyphs)
                ruleAsSubtable.Input = [
                    st.InputClassDef.classDefs[list(x)[0]]
                    for x in rule.glyphs[1:]
                ]
        ...
        setattr(
            st,
            self.ruleSetAttr_(format=2, chaining=chaining) + "Count",
            len(classSets),
        )
        st.Coverage = buildCoverage(coverage, self.glyphMap)
        return st
    ...
    def ruleSetAttr_(self, format=1, chaining=True):
        ...
        subtablename = (
            f"{self.subtable_type[0:3]}{formatType}Set"  # Sub, not Subst.
        )
        if chaining:
            subtablename = "Chain" + subtablename
        return subtablename
    ...
    def ruleAttr_(self, format=1, chaining=True):
        if format == 1:
            ...
            raise AssertionError(formatType)
        subtablename = (
            f"{self.subtable_type[0:3]}{formatType}Rule"  # Sub, not Subst.
        )
        if chaining:
            subtablename = "Chain" + subtablename
        return subtablename
        ...
        for rule in self.rules[::-1]:
            ...
            for sub in rule.lookups:
                if isinstance(sub, builder_class) and not any(
                    g in mapping and mapping[g] != sub.mapping[g]
                    for g in sub.mapping
                ):
                    res = sub
        return res
...
class MultipleSubstBuilder(LookupBuilder):
    ...
    def equals(self, other):
        return (
            LookupBuilder.equals(self, other) and self.mapping == other.mapping
        )
...
class CursivePosBuilder(LookupBuilder):
    ...
    def build(self):
        ...
        subtables = [
            buildCursivePosSubtable(s, self.glyphMap) for s in attachments
        ]
        return self.buildLookup_(subtables)
        ...
        for subtable in self.get_subtables_():
            ...
            for mark, (mc, anchor) in subtable[0].items():
                if mc not in markClasses:
                    raise ValueError(
                        "Mark class %s not found for mark glyph %s"
                        % (mc, mark)
                    )
            ...
            for glyph, anchors in subtable[1].items():
                ...
                for mc, anchor in anchors.items():
                    if mc not in markClasses:
                        raise ValueError(
                            "Mark class %s not found for base glyph %s"
                            % (mc, glyph)
                        )
                    bases[glyph][markClasses[mc]] = anchor
            subtables.append(
                buildMarkBasePosSubtable(marks, bases, self.glyphMap)
            )
        return self.buildLookup_(subtables)
    ...
    def build(self):
        ...
        for subtable in self.get_subtables_():
            ...
            for lig, components in subtable[1].items():
                ...
                for c in components:
                    ligs[lig].append(
                        {markClasses[mc]: a for mc, a in c.items()}
                    )
            subtables.append(
                buildMarkLigPosSubtable(marks, ligs, self.glyphMap)
            )
        return self.buildLookup_(subtables)
    ...
    def promote_lookup_type(self, is_named_lookup):
        ...
        if to_single:
            ...
            for builder_class, (key, value) in zip(
                ...
                if (
                    curr_builder is None
                    or type(curr_builder) is not builder_class
                ):
...
@deprecateFunction(
    "use buildMarkLigPosSubtable() instead", category=DeprecationWarning
)
...
def buildPairPosClassesSubtable(
    pairs, glyphMap, valueFormat1=None, valueFormat2=None
):
    ...
    self = ot.PairPos()
    self.Format = 2
    valueFormat1 = self.ValueFormat1 = _getValueFormat(
        valueFormat1, pairs.values(), 0
    )
    valueFormat2 = self.ValueFormat2 = _getValueFormat(
        valueFormat2, pairs.values(), 1
    )
...
def buildPairPosGlyphsSubtable(
    ...
    for glyph in self.Coverage.glyphs:
        ...
        for glyph2, val1, val2 in sorted(
            p[glyph], key=lambda x: glyphMap[x[0]]
        ):
...
def buildAttachList(attachPoints, glyphMap):
    ...
    self.AttachPoint = [
        buildAttachPoint(attachPoints[g]) for g in self.Coverage.glyphs
    ]
...
def buildLigGlyph(coords, points):
    ...
    if coords:
        coords = sorted(
            coords, key=lambda c: c[0] if isinstance(c, tuple) else c
        )
...
def _buildAxisValuesFormat4(
    locations, axes, ttFont, windowsNames=True, macNames=True
):
    axisTagToIndex = {}
    for axisRecordIndex, axisDict in enumerate(axes):
...
def _buildMathGlyphInfo(
    ...
    if not any(
        [extendedShapes, italicsCorrections, topAccentAttachments, mathKerns]
    ):
        return None
    ...
    if italicsCorrections:
        ...
        info.MathItalicsCorrectionInfo.ItalicsCorrectionCount = len(
            coverage.glyphs
        )
    ...
    if topAccentAttachments:
        ...
        info.MathTopAccentAttachment.TopAccentAttachmentCount = len(
            coverage.glyphs
        )
