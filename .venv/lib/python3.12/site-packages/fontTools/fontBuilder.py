...
class FontBuilder(object):
    def __init__(
        self, unitsPerEm=None, font=None, isTTF=True, glyphDataFormat=0
    ):
    ...
    def setupCharacterMap(self, cmapping, uvs=None, allowFallback=False):
        ...
        if highestUnicode > 0xFFFF:
            cmapping_3_1 = dict(
                (k, v) for k, v in cmapping.items() if k < 0x10000
            )
    ...
    def setupCFF2(self, charStringsDict, fdArrayList=None, regions=None):
        ...
        charStrings = CharStrings(
            None, None, globalSubrs, private, fdSelect, fdArray
        )
        for glyphName, charString in charStringsDict.items():
            charString.private = private
            charString.globalSubrs = globalSubrs
    ...
    def setupGlyf(
        self, glyphs, calcGlyphBounds=True, validateGlyphFormat=True
    ):
        ...
        if validateGlyphFormat and self.font["head"].glyphDataFormat == 0:
            for name, g in glyphs.items():
                if g.numberOfContours > 0 and any(
                    f & flagCubic for f in g.flags
                ):
                    raise ValueError(
    ...
    def setupVerticalOrigins(
        self, verticalOrigins, defaultVerticalOrigin=None
    ):
    ...
    def addOpenTypeFeatures(
        self, features, filename=None, tables=None, debug=False
    ):
    ...
    def addFeatureVariations(
        self, conditionalSubstitutions, featureTag="rvrn"
    ):
        ...
        if "fvar" not in self.font:
            raise KeyError(
                "'fvar' table is missing; can't add FeatureVariations."
            )
