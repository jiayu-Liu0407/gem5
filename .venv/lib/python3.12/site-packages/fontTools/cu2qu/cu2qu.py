...
@cython.inline
@cython.locals(
    a=cython.complex, b=cython.complex, c=cython.complex, d=cython.complex
)
@cython.locals(
...
@cython.locals(
    ab=cython.complex, cd=cython.complex, p=cython.complex, h=cython.double
)
...
def cubic_approx_quadratic(cubic, tolerance):
    ...
    c1 = c0 + (q1 - c0) * (2 / 3)
    c2 = c3 + (q1 - c3) * (2 / 3)
    if not cubic_farthest_fit_inside(
        0, c1 - cubic[1], c2 - cubic[2], 0, tolerance
    ):
        return None
    return c0, q1, c3
...
def curves_to_quadratic(curves, max_errors, all_quadratic=True):
    ...
    n = 1
    while True:
        spline = cubic_approx_spline(
            curves[i], n, max_errors[i], all_quadratic
        )
        if spline is None:
            if n == MAX_N:
                break
