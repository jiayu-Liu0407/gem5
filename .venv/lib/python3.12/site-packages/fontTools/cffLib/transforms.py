...
class _DehintingT2Decompiler(T2WidthExtractor):
    ...
    def execute(self, charString):
        old_hints = (
            charString._hints if hasattr(charString, "_hints") else None
        )
...
def _cs_subset_subroutines(charstring, subrs, gsubrs):
    ...
    for i in range(1, len(p)):
        if p[i] == "callsubr":
            ...
            p[i - 1] = (
                subrs._used.index(p[i - 1] + subrs._old_bias) - subrs._new_bias
            )
        elif p[i] == "callgsubr":
            assert isinstance(p[i - 1], int)
            p[i - 1] = (
                gsubrs._used.index(p[i - 1] + gsubrs._old_bias)
                - gsubrs._new_bias
            )
    ...
    for fontname in cff.keys():
        ...
        for subrs in all_subrs:
            if subrs == font.GlobalSubrs:
                if not hasattr(font, "FDArray") and hasattr(
                    font.Private, "Subrs"
                ):
