...
class _GeneralizerDecombinerCommandsMap(object):
    ...
    def hhcurveto(args):
        ...
        if l % 2 == 1:
            yield (
                "rrcurveto",
                [args[1], args[0], args[2], args[3], args[4], 0],
            )
            args = args[5:]
    ...
    def vvcurveto(args):
        ...
        if l % 2 == 1:
            yield (
                "rrcurveto",
                [args[0], args[1], args[2], args[3], 0, args[4]],
            )
            args = args[5:]
        for args in _everyN(args, 4):
    ...
    def hvcurveto(args):
        ...
        if last_args:
            ...
            if lastStraight:
                yield (
                    "rrcurveto",
                    [args[0], 0, args[1], args[2], args[4], args[3]],
                )
            else:
                yield (
                    "rrcurveto",
                    [0, args[0], args[1], args[2], args[3], args[4]],
                )
...
def _convertBlendOpToArgs(blendList):
    ...
    deltaList = [
        deltaArgs[i : i + numRegions]
        for i in range(0, numDeltaValues, numRegions)
    ]
    ...
    if isinstance(b, list):
        if isinstance(a, list):
            if len(a) != len(b) or a[-1] != b[-1]:
                raise ValueError()
            return [_addArgs(va, vb) for va, vb in zip(a[:-1], b[:-1])] + [
                a[-1]
            ]
        else:
            a, b = b, a
...
def specializeCommands(
    ...
    if not preserveTopology:
        for i in range(len(commands) - 1, -1, -1):
            ...
            if (
                i
                and op in {"hlineto", "vlineto"}
                and (op == commands[i - 1][0])
            ):
    ...
    for i in range(len(commands)):
        ...
        if op[2:] == "curveto" and op[:2] not in {
            "rr",
            "hh",
            "vv",
            "vh",
