...
class CFFFontSet(object):
    ...
    def fromXML(self, name, attrs, content, otFont=None):
        ...
        if name == "CFFFont":
            ...
            if (
                hasattr(topDict, "VarStore")
                and topDict.FDArray[0].vstore is None
            ):
...
class GlobalSubrsIndex(Index):
    ...
    def produceItem(self, index, data, file, offset):
        if self.private is not None:
            ...
            private = None
        return self.subrClass(
            data, private=private, globalSubrs=self.globalSubrs
        )
...
class TopDictIndex(Index):
    ...
    def __init__(
        self, file=None, cff2GetGlyphOrder=None, topSize=0, isCFF2=None
    ):
    ...
    def fromXML(self, name, attrs, content):
        for element in content:
            ...
            charString = charStringClass(
                private=private, globalSubrs=self.globalSubrs
            )
        ...
        if value > 2:
            ...
            if format == 0:
                ...
                charset = parseCharset(
                    numGlyphs, file, parent.strings, isCID, format
                )
            else:
...
class DictCompiler(object):
    ...
    def compile(self, reason):
        log.log(
            DEBUG, "-- compiling %s for %s", self.__class__.__name__, reason
        )
...
class FontDictCompiler(DictCompiler):
    ...
    def __init__(self, dictObj, strings, parent, isCFF2=None):
        super(FontDictCompiler, self).__init__(
            dictObj, strings, parent, isCFF2=isCFF2
        )
        #
...
class BaseDict(object):
    ...
    def decompile(self, data):
        log.log(
            DEBUG, "    length %s is %d", self.__class__.__name__, len(data)
        )
...
class PrivateDict(BaseDict):
    ...
    def __init__(
        self, strings=None, file=None, offset=None, isCFF2=None, vstore=None
    ):
...
class IndexedStrings(object):
    ...
    def __init__(self, file=None):
        if file is None:
            ...
            strings = [
                tostr(s, encoding="latin1") for s in Index(file, isCFF2=False)
            ]
        self.strings = strings
    ...
    def buildStringMapping(self):
        ...
        for index in range(len(self.strings)):
            self.stringMapping[self.strings[index]] = (
