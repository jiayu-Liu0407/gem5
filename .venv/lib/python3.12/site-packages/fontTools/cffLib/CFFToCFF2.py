...
def _convertCFFToCFF2(cff, otFont):
    ...
    localSubrs = (
        ...
        if fdArray
            ...
            and hasattr(topDict.Private, "Subrs")
            else []
        )
    )
    ...
    for glyphName in charStrings.keys():
        ...
        try:
            ...
            while len(program) >= 2 and program[1] in [
                "callsubr",
                "callgsubr",
            ]:
...
def main(args=None):
    ...
    font = TTFont(
        infile, recalcTimestamp=options.recalc_timestamp, recalcBBoxes=False
    )
