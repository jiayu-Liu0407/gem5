...
def buildClipList(clipBoxes: Dict[str, _ClipBoxInput]) -> ot.ClipList:
    ...
    clipList.Format = 1
    clipList.clips = {
        name: buildClipBox(box) for name, box in clipBoxes.items()
    }
    return clipList
...
def buildPaletteLabels(
    labels: Iterable[_OptionalLocalizedString],
    nameTable: _n_a_m_e.table__n_a_m_e,
) -> List[Optional[int]]:
    return [
        (
...
def buildCPAL(
    ...
    for i, palette in enumerate(palettes):
        ...
        for j, color in enumerate(palette):
            ...
            colors.append(
                C_P_A_L_.Color(
                    *(round(v * 255) for v in (blue, green, red, alpha))
                )
            )
    ...
    if any(
        ...
        if paletteTypes is not None:
            ...
            cpal.paletteTypes = [
                C_P_A_L_.table_C_P_A_L_.DEFAULT_PALETTE_TYPE
            ] * len(palettes)
        ...
        if paletteLabels is not None:
            ...
            cpal.paletteLabels = [C_P_A_L_.table_C_P_A_L_.NO_NAME_ID] * len(
                palettes
            )
...
def buildColrV1(
    ...
    *,
    ...
    for baseGlyph, paint in colorGlyphItems:
        try:
            baseGlyphs.append(
                buildBaseGlyphPaintRecord(baseGlyph, layerBuilder, paint)
            )
