...
class VoltToFea:
    ...
    def _collectStatements(
        self, doc, tables, ignore_unsupported_settings=False
    ):
        ...
        groups = [
            s for s in doc.statements if isinstance(s, VAst.GroupDefinition)
        ]
        ...
        for statement in doc.statements:
            if isinstance(statement, VAst.AnchorDefinition):
                ...
                statement, (VAst.GlyphDefinition, VAst.GroupDefinition)
            ):
                pass  # Handled above
    ...
    def _buildFeatureFile(self, tables):
        ...
        for feature_tag in features:
            ...
            for script_tag in scripts:
                ...
                for language_tag in langs:
                    langs[language_tag] = [
                        l
                        for l in langs[language_tag]
                        if l.lower() in self._lookups
                    ]
        ...
        if features:
            ...
            for feature_tag, scripts in features.items():
                ...
                script_tags = sorted(
                    scripts, key=lambda k: 0 if k == "DFLT" else 1
                )
                ...
                for script_tag in script_tags:
                    if feature_tag != "aalt":
                        feature.statements.append(
                            ast.ScriptStatement(script_tag)
                        )
                    ...
                    for language_tag in language_tags:
                        if feature_tag != "aalt":
                            include_default = (
                                True if language_tag == "dflt" else False
                            )
                            feature.statements.append(
    ...
    def _gposContextLookup(
        self, lookup, prefix, suffix, ignore, fealookup, chained
    ):
        statements = fealookup.statements
            ...
            for key, val in sub.mapping.items():
                ...
                for src_glyph, repl_glyph in zip(
                    ...
                    alternates.setdefault(
                        str(self._glyphName(src_glyph)), []
                    ).append(str(self._glyphName(repl_glyph)))
        ...
        for key, val in sub.mapping.items():
            ...
            if isinstance(sub, VAst.SubstitutionSingleDefinition):
                ...
                if n > 1:
                    ...
                    glyphs = [
                        ([x[0]] * n if len(x) == 1 else x) for x in glyphs
                    ]
    ...
    def _gsubContextLookup(
        ...
        if ignore:
            statements.append(
                ast.IgnoreSubstStatement([(prefix, glyphs, suffix)])
            )
        else:
            statements.append(
