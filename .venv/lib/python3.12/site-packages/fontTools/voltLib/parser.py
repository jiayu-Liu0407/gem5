...
class Parser(object):
    ...
    def parse_def_glyph_(self):
        ...
        if self.next_token_ == "UNICODE":
            ...
            if gunicode[0] < 0:
                raise VoltLibError(
                    "Invalid glyph UNICODE", self.cur_token_location_
                )
    ...
    def parse_langsys_(self):
        ...
        def_langsys = ast.LangSysDefinition(
            name, tag, features, location=location
        )
        return def_langsys
        ...
        if max_src == 1 and max_dest == 1:
            ...
            if len(set(glyphs)) != len(glyphs):  # src has duplicates
                ...
                if reversal:
                    ...
                    sub = ast.SubstitutionSingleDefinition(
                        mapping, location=location
                    )
        elif max_src == 1 and max_dest > 1:
            sub = ast.SubstitutionMultipleDefinition(
                mapping, location=location
            )
        elif max_src > 1 and max_dest == 1:
            sub = ast.SubstitutionLigatureDefinition(
                mapping, location=location
            )
        return sub
