...
class _UFOBaseIO:
    def getFileModificationTime(self, path):
        ...
        try:
            dt = self.fs.getinfo(
                fsdecode(path), namespaces=["details"]
            ).modified
    ...
    def _writePlist(self, fileName, obj):
        ...
        if self._havePreviousFile:
            ...
            if self.fs.exists(fileName) and data == self.fs.readbytes(
                fileName
            ):
                return
...
class UFOReader(_UFOBaseIO):
        ...
        if isinstance(path, str):
            ...
            try:
                if structure is UFOFileStructure.ZIP:
                    parentFS = fs.zipfs.ZipFS(
                        path, write=False, encoding="utf-8"
                    )
                else:
        ...
        if self._upConvertedKerningData:
            ...
            if validate:
                invalidFormatMessage = (
                    "groups.plist is not properly formatted."
                )
    ...
    def _readMetaInfo(self, validate=None):
        ...
        try:
            formatVersion = UFOFormatVersion(
    ...
    def readInfo(self, info, validate=None):
        ...
        if self._formatVersion == UFOFormatVersion.FORMAT_1_0:
            ...
            infoDataToSet = _convertFontInfoDataVersion2ToVersion3(
                infoDataToSet
            )
        # version 2
    ...
    def readFeatures(self):
        ...
        try:
            with self.fs.open(
                FEATURES_FILENAME, "r", encoding="utf-8-sig"
            ) as f:
                return f.read()
        except fs.errors.ResourceNotFound:
            return ""
    ...
    def getGlyphSet(
        self, layerName=None, validateRead=None, validateWrite=None
    ):
        """
    ...
    def getDataDirectoryListing(self):
        ...
        try:
            ...
            raise UFOLibError(
                'The UFO contains a "data" file instead of a directory.'
            )
        try:
...
class UFOWriter(UFOReader):
    ...
    def __init__(
        ...
        if isinstance(path, str):
            ...
            if havePreviousFile:
                ...
                if structure is not None:
                    try:
                        ...
                        raise UFOLibError(
                            "Invalid or unsupported structure: '%s'"
                            % structure
                        )
            ...
            if structure is UFOFileStructure.ZIP:
                if havePreviousFile:
                    ...
                    rootDir = (
                        os.path.splitext(os.path.basename(path))[0] + ".ufo"
                    )
                    parentFS = fs.zipfs.ZipFS(
                        path, write=True, encoding="utf-8"
                    )
                    parentFS.makedir(rootDir)
                    self.fs = parentFS.opendir(
                        rootDir, factory=fs.subfs.ClosingSubFS
                    )
            else:
        ...
        self.layerContents = {}
        if (
            previousFormatVersion is not None
            and previousFormatVersion.major >= 3
        ):
            # already exists
    ...
    def writeGroups(self, groups, validate=None):
        ...
        if groupsNew:
            ...
            self.removePath(
                GROUPS_FILENAME, force=True, removeEmptyParents=False
            )
    ...
    def writeKerning(self, kerning, validate=None):
        ...
        if kerningDict:
            ...
            self.removePath(
                KERNING_FILENAME, force=True, removeEmptyParents=False
            )
    ...
    def writeFeatures(self, features, validate=None):
        ...
        if self._formatVersion == UFOFormatVersion.FORMAT_1_0:
            raise UFOLibError(
                "features.fea is not allowed in UFO Format Version 1."
            )
        if validate:
        ...
        if features:
            ...
            self.removePath(
                FEATURES_FILENAME, force=True, removeEmptyParents=False
            )
    ...
    def _findDirectoryForLayerName(self, layerName):
        foundDirectory = None
        for existingLayerName, directoryName in list(
            self.layerContents.items()
        ):
    ...
    def getGlyphSet(
        ...
        if layerName is None and defaultLayer:
            ...
            raise UFOLibError(
                "A layer name must be provided for non-default layers."
            )
    ...
    def renameGlyphSet(self, layerName, newLayerName, defaultLayer=False):
        ...
        if layerName == newLayerName:
            ...
            if newLayerName in self.layerContents:
                raise UFOLibError(
                    "A layer named %s already exists." % newLayerName
                )
            # make sure the default layer doesn't already exist
            if (
                defaultLayer
                and DEFAULT_GLYPHS_DIRNAME in self.layerContents.values()
            ):
    ...
    def removeImage(
        self, fileName, validate=None
    ):  # XXX remove unused 'validate'?
        """
    ...
    def copyImageFromReader(
        self, reader, sourceFileName, destFileName, validate=None
    ):
        """
    ...
    def close(self):
        if (
            ...
            and self._fileStructure is UFOFileStructure.ZIP
        ):
            ...
            rootDir = (
                os.path.splitext(os.path.basename(self._path))[0] + ".ufo"
            )
            with fs.zipfs.ZipFS(
                self._path, write=True, encoding="utf-8"
            ) as destFS:
...
def _sniffFileStructure(ufo_path):
    ...
    if zipfile.is_zipfile(ufo_path):
        ...
        raise UFOLibError(
            "The specified UFO does not have a known structure: '%s'"
            % ufo_path
        )
    else:
...
def validateInfoVersion2Data(infoData):
    ...
    for attr, value in list(infoData.items()):
        ...
        if not isValidValue:
            raise UFOLibError(
                f"Invalid value for attribute {attr} ({value!r})."
            )
        else:
...
deprecatedFontInfoAttributesVersion2 = set(
    fontInfoAttributesVersion1To2.keys()
)
...
def _convertFontInfoDataVersion3ToVersion2(data):
    ...
    for attr, value in list(data.items()):
        (
            newAttr,
            newValue,
        ) = convertFontInfoValueForAttributeFromVersion3ToVersion2(attr, value)
        if newAttr not in fontInfoAttributesVersion2:
            continue
        converted[newAttr] = newValue
    return converted
