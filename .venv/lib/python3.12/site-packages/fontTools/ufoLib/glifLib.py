...
class GlyphSet(_UFOBaseIO):
    ...
    def __init__(
        ...
        if isinstance(path, str):
            ...
            try:
                ...
                raise GlifLibError(
                    "the filesystem '%s' is closed" % filesystem
                )
    ...
    def rebuildContents(self, validateRead=None):
        ...
        if validateRead:
            ...
            if invalidFormat:
                raise GlifLibError(
                    "%s is not properly formatted" % CONTENTS_FILENAME
                )
    ...
    def readGlyph(
        self, glyphName, glyphObject=None, pointPen=None, validate=None
    ):
        """
    ...
    def writeGlyph(
        ...
        if fileName is None:
            ...
            fileName = self.glyphNameToFileName(
                glyphName, self._existingFileNames
            )
...
def glyphNameToFileName(glyphName, existingFileNames):
    ...
    return userNameToFileName(
        glyphName, existing=existingFileNames, suffix=".glif"
    )
...
def _writeGuidelines(glyphObject, element, identifiers, validate):
    ...
    if validate and not guidelinesValidator(guidelines):
        raise GlifLibError(
            "guidelines attribute does not have the proper structure."
        )
...
def _writeAnchorsFormat1(pen, anchors, validate):
    if validate and not anchorsValidator(anchors):
        raise GlifLibError(
            "anchors attribute does not have the proper structure."
        )
    ...
    try:
        if etree._have_lxml:
            root = etree.fromstring(
                data, parser=etree.XMLParser(remove_comments=True)
            )
        else:
    ...
    for element in tree:
        if element.tag == "outline":
            if validate:
                if haveSeenOutline:
                    raise GlifLibError(
                        "The outline element occurs more than once."
                    )
                if element.attrib:
                    raise GlifLibError(
            ...
            if validate and haveSeenAdvance:
                raise GlifLibError(
                    "The advance element occurs more than once."
                )
...
def _readGlyphFromTreeFormat2(
    ...
    guidelines = []
    anchors = []
    haveSeenAdvance = haveSeenImage = haveSeenOutline = haveSeenLib = (
        haveSeenNote
    ) = False
    identifiers = set()
    for element in tree:
        if element.tag == "outline":
            ...
            if validate:
                if haveSeenImage:
                    raise GlifLibError(
                        "The image element occurs more than once."
                    )
                if len(element):
...
def buildOutlineFormat1(glyphObject, pen, outline, validate):
    ...
    for element in outline:
        if element.tag == "contour":
            ...
            raise GlifLibError(
                "Unknown element in outline element: %s" % element
            )
    ...
    if validate:
        ...
        for attr in component.attrib.keys():
            if attr not in componentAttributesFormat1:
                raise GlifLibError(
                    "Unknown attribute in component element: %s" % attr
                )
    baseGlyphName = component.get("base")
    if validate and baseGlyphName is None:
        raise GlifLibError(
            "The base attribute is not defined in the component."
        )
    transformation = []
    for attr, default in _transformationInfo:
...
def buildOutlineFormat2(glyphObject, pen, outline, identifiers, validate):
    for element in outline:
        if element.tag == "contour":
            ...
            raise GlifLibError(
                "Unknown element in outline element: %s" % element.tag
            )
    ...
    if validate:
        for attr in contour.attrib.keys():
            if attr not in contourAttributesFormat2:
                raise GlifLibError(
                    "Unknown attribute in contour element: %s" % attr
                )
    ...
    for point in contour:
        ...
        if identifier is not None:
            if validate:
                if identifier in identifiers:
                    raise GlifLibError(
                        "The identifier %s is used more than once."
                        % identifier
                    )
                if not identifierValidator(identifier):
                    raise GlifLibError(
                        "The identifier %s is not valid." % identifier
                    )
            identifiers.add(identifier)
        try:
            pen.addPoint(
            ...
            pen.addPoint(
                (x, y), segmentType=segmentType, smooth=smooth, name=name
            )
            warn(
...
def _buildOutlineComponentFormat2(pen, component, identifiers, validate):
    ...
    try:
        pen.addComponent(
            baseGlyphName, tuple(transformation), identifier=identifier
        )
    except TypeError:
        pen.addComponent(baseGlyphName, tuple(transformation))
        warn(
        ...
        if validate:
            ...
            for attr in point.keys():
                if attr not in pointAttributes:
                    raise GlifLibError(
                        "Unknown attribute in point element: %s" % attr
                    )
            ...
            if segmentType is None:
                ...
                if offCurvesCount:
                    ...
                    if segmentType == "move":
                        ...
                        if offCurvesCount > 2:
                            raise GlifLibError(
                                "Too many offcurves defined for curve."
                            )
...
class GLIFPointPen(AbstractPointPen):
        ...
        **kwargs,
        ...
        if self.validate:
            ...
            if (
                ...
                raise GlifLibError(
                    "offcurve occurs before %s point." % segmentType
                )
            if segmentType == "curve" and self.prevOffCurveCount > 2:
                raise GlifLibError(
                    "too many offcurve points before curve point."
                )
    ...
    def addComponent(
        self, glyphName, transformation, identifier=None, **kwargs
    ):
