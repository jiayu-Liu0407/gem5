...
def layerContentsValidator(value, ufoPathOrFileSystem):
    ...
    for entry in value:
        ...
        if layerName in usedLayerNames:
            return (
                False,
                "The layer name %s is used by more than one layer."
                % layerName,
            )
        ...
        if directoryName in usedDirectories:
            return (
                False,
                "The directory %s is used by more than one layer."
                % directoryName,
            )
    ...
    for groupName, glyphList in value.items():
        ...
        if groupName.startswith("public."):
            if not groupName.startswith(
                "public.kern1."
            ) and not groupName.startswith("public.kern2."):
            ...
            for glyphName in glyphList:
                if not isinstance(glyphName, str):
                    return (
                        False,
                        "The group data %s contains an invalid member."
                        % groupName,
                    )
                if glyphName in d:
                    return (
...
def fontLibValidator(value):
    ...
    for key, value in value.items():
        ...
        if key == "public.glyphOrder":
            bogusGlyphOrderMessage = (
                "public.glyphOrder is not properly formatted: %s"
            )
            if not isinstance(value, (list, tuple)):
                reason = (
                    "expected list or tuple, found %s" % type(value).__name__
                )
                return False, bogusGlyphOrderMessage % reason
            for glyphName in value:
                if not isinstance(glyphName, str):
                    reason = (
                        "expected str, found %s" % type(glyphName).__name__
                    )
                    return False, bogusGlyphOrderMessage % reason
    return True, None
