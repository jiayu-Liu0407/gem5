...
class TestUfunc:
    ...
    @pytest.mark.parametrize(
        ...
        [
            None,
            np.array(
                [[1.0 + 1j, 0.5, -0.5j], [0.25, 2j, 0.0], [4.0, 0.0, -1j]]
            ),
        ],
    )
    def test_vecmatvec_identity(self, matrix, vec):
        ...
        assert_array_equal(
            vecmat, (vec[..., np.newaxis].mT.conj() @ mat).squeeze(-2)
        )
    ...
    def test_vecdot_matvec_vecmat_complex(self, ufunc, shape1, shape2, conj):
        ...
        if not conj:
            assert_array_equal(
                arr1.reshape(shape1) @ arr2.reshape(shape2), expected1
            )
    ...
    def test_array_wrap_array_priority(self):
        ...
        assert (
            np.add(np.zeros(2), ArrayPriorityMinus0(2)) is ArrayPriorityMinus0
        )
