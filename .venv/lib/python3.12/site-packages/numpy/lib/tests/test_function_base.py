...
class TestDiff:
    ...
    def test_n(self):
        ...
        for n, (expected_n, output_n) in enumerate(
            zip(expected, output), start=1
        ):
...
class TestTrimZeros:
    ...
    def test_unexpected_trim_value(self, trim):
        arr = self.a
        with pytest.raises(
            ValueError, match=r"unexpected character\(s\) in `trim`"
        ):
...
class TestInterp:
    ...
    def test_non_finite_half_inf_x(self, sc):
        ...
        assert_equal(
            np.interp(0.5, [-np.inf, 1], sc([0, 10])), sc(10)
        )  # noqa: E202
