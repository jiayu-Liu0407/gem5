...
def test_get_names() -> None:
    ...
    names = rfn.get_names(
        np.empty((1,), dtype=[("A", int), ("B", float)]).dtype
    )
...
def test_stack_arrays() -> None:
    ...
    zz = np.ones(
        (int(2),), [("A", "|S3"), ("B", np.float64), ("C", np.float64)]
    )
    assert_type(
...
def test_find_duplicates() -> None:
    ...
    assert_type(
        rfn.find_duplicates(a), np.ma.MaskedArray[Any, np.dtype[np.void]]
    )
    assert_type(
