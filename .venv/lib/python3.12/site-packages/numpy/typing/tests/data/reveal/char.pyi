...
assert_type(
    np.char.ljust(AR_T, [4, 2, 1], fillchar=["a", "b", "c"]), AR_TU_alias
)
...
assert_type(
    np.char.rjust(AR_T, [4, 2, 1], fillchar=["a", "b", "c"]), AR_TU_alias
)
...
assert_type(
    np.char.startswith(AR_T, "a", start=[1, 2, 3]), npt.NDArray[np.bool]
)
...
assert_type(
    np.char.array(AR_U), np.char.chararray[np_t._AnyShape, np.dtype[np.str_]]
)
assert_type(
...
assert_type(
    np.char.asarray(AR_U), np.char.chararray[np_t._AnyShape, np.dtype[np.str_]]
)
assert_type(
