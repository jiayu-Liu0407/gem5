...
assert_type(
    divmod(m_td, m_td), tuple[np.int64, np.timedelta64[dt.timedelta | None]]
)

assert_type(
    divmod(AR_m, m), tuple[npt.NDArray[np.int64], npt.NDArray[np.timedelta64]]
)
assert_type(
    divmod(m, AR_m), tuple[npt.NDArray[np.int64], npt.NDArray[np.timedelta64]]
)
...
assert_type(
    divmod(i8, i4), tuple[np.int64, np.int64] | tuple[np.int32, np.int32]
)
...
assert_type(
    divmod(i8, AR_b), tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]]
)
...
assert_type(
    divmod(i4, i8), tuple[np.int64, np.int64] | tuple[np.int32, np.int32]
)
assert_type(divmod(i4, i4), tuple[np.int32, np.int32])
# workarounds for https://github.com/microsoft/pyright/issues/9663
assert_type(
...
assert_type(
    AR_b.__divmod__(i8), tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]]
)
...
assert_type(
    divmod(f8, AR_b), tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]]
)
