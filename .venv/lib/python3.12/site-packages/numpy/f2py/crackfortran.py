...
def undo_rmbadname1(name):
    if name in invbadnames:
        errmess(
            f'undo_rmbadname1: Replacing "{name}" with "{invbadnames[name]}".\n'
        )
...
endifs = (
    r"end\s*(if|do|where|select|while|forall|associate|" r"critical|enum|team)"
)
endifpattern = (
...
def analyzeline(m, case, line):
    ...
    if case in ["begin", "call", "callfun"]:
        ...
        for e in [x.strip() for x in markoutercomma(ll).split("@,@")]:
            ...
            if case in ["public", "private"] and k in {
                "operator",
                "assignment",
            }:
                k += m1.group("after")
        ...
        for l in ll:
            ...
            for idx, v in enumerate(
                ...
                try:
                    new_val = (
                        f"(/{', '.join(matches)}/)" if vdim else matches[idx]
                    )
                except IndexError:
                    # gh-24746
...
def crack2fortrangen(block, tab="\n", as_interface=False):
    ...
    if "entry" in block:
        ...
        for k, i in list(block["entry"].items()):
            entry_stmts = (
                f"{entry_stmts}{tab + tabchar}entry {k}({','.join(i)})"
            )
