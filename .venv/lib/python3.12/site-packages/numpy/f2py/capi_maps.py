...
def load_f2cmap_file(f2cmap_file):
    ...
    try:
        ...
        errmess(
            f"Failed to apply user defined f2cmap changes: {msg}. Skipping.\n"
        )
...
def sign2map(a, var):
    ...
    if debugcapi(var):
        ...
        if isexternal(var):
            ret["vardebuginfo"] = (
                f"debug-capi:{a}=>{ret['cbname']}:{','.join(rl)}"
            )
        else:
        ...
        if isscalar(var):
            if ret["ctype"] in cformat_map:
                ret["vardebugshowvalue"] = (
                    f"debug-capi:{a}={cformat_map[ret['ctype']]}"
                )
        if isstring(var):
            ret["vardebugshowvalue"] = (
...
def cb_routsign2map(rout, um):
    ...
    if isintent_callback(rout):
        if "_" in rout["name"]:
            ...
            F_FUNC = "F_FUNC"
        ret["callbackname"] = (
            f"{F_FUNC}({rout['name'].lower()},{rout['name'].upper()})"
        )
        ret["static"] = "extern"
    else:
