...
class ImageFileDirectory_v2(_IFDv2Base):
    ...
    def tobytes(self, offset: int = 0) -> bytes:
        ...
        offset += (
            len(result)
            + len(self._tags_v2) * (20 if self._bigtiff else 12)
            + fmt_size
        )
        stripoffsets = None
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            msg = (
                f"save: {tagname} ({tag}) - type: {typname} ({typ}) - value: "
            )
            msg += (
                f"<table: {len(data)} bytes>"
                if len(data) >= 16
                else str(values)
            )
            logger.debug(msg)
            ...
            if len(data) <= fmt_size:
                entries.append(
                    (tag, typ, count, data.ljust(fmt_size, b"\0"), b"")
                )
            else:
                entries.append(
                    (tag, typ, count, self._pack(fmt, offset), data)
                )
