...
class MplRenderer(Renderer):
    ...
    def title(
        self, title: str, ax: Axes | int = 0, color: str | None = None
    ) -> None:
...
class MplDebugRenderer(MplRenderer):
    ...
    def filled(
        ...
        ax = self._get_ax(ax)
        filled = convert_filled(
            filled, fill_type, FillType.ChunkCombinedOffset
        )
        ...
        if line_color is not None:
            for points, offsets in zip(*filled):
                ...
                for start, end in pairwise(offsets):
                    ...
                    ax.plot(
                        xys[:, 0], xys[:, 1], c=line_color, alpha=line_alpha
                    )
        ...
        if arrow_size > 0.0:
            for line in separate_lines:
                for i in range(len(line) - 1):
                    self._arrow(
                        ax, line[i], line[i + 1], color, alpha, arrow_size
                    )
