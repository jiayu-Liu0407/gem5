...
def simple(
    ...
    CoordinateArray,
    CoordinateArray,
    CoordinateArray | np.ma.MaskedArray[Any, Any],
]:
    ...
    mid = np.asarray(
        [[0.4, 0.2], [0.3, 0.8], [0.9, 0.75], [0.7, 0.3], [0.05, 0.7]]
    )
    ...
    if want_mask:
        mask = np.logical_or(
            ((x / xscale - 1.0) ** 2 / 0.2 + (y / yscale - 0.0) ** 2 / 0.1)
            < 1.0,
            ((x / xscale - 0.2) ** 2 / 0.02 + (y / yscale - 0.45) ** 2 / 0.08)
            < 1.0,
        )
