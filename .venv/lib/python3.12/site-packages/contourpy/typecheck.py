...
def check_code_array(codes: Any) -> None:
    ...
    if not (codes.ndim == 1 and len(codes) > 1):
        raise ValueError(
            f"Expected numpy array of shape (?,) not {codes.shape}"
        )
    if codes[0] != MOVETO:
        raise ValueError(
...
def check_offset_array(offsets: Any) -> None:
    ...
    if not (offsets.ndim == 1 and len(offsets) > 1):
        raise ValueError(
            f"Expected numpy array of shape (?,) not {offsets.shape}"
        )
    if offsets[0] != 0:
        raise ValueError(
            f"First element of offset array must be 0, not {offsets[0]}"
        )
...
def check_point_array(points: Any) -> None:
    ...
    if not (points.ndim == 2 and points.shape[1] == 2 and points.shape[0] > 1):
        raise ValueError(
            f"Expected numpy array of shape (?, 2) not {points.shape}"
        )
    ...
    if fill_type == FillType.OuterCode:
        ...
        for i, (points, offsets) in enumerate(zip(*filled)):
            ...
            if offsets[-1] != len(points):
                raise ValueError(
                    f"Inconsistent points and offsets in polygon {i}"
                )
    elif fill_type == FillType.ChunkCombinedCode:
        if TYPE_CHECKING:
            filled = cast(cpy.FillReturn_ChunkCombinedCode, filled)
        _check_tuple_of_lists_with_same_length(
            filled, 2, allow_empty_lists=False
        )
        ...
        for chunk, (points_or_none, offsets_or_none) in enumerate(
            zip(*filled)
        ):
        ...
        _check_tuple_of_lists_with_same_length(
            filled, 3, allow_empty_lists=False
        )
        for i, (
            points_or_none,
            codes_or_none,
            outer_offsets_or_none,
        ) in enumerate(zip(*filled)):
            if (
        ...
        for i, (
            ...
            if (
                ...
                if offsets_or_none[-1] != len(points_or_none):
                    raise ValueError(
                        f"Inconsistent points and offsets in chunk {i}"
                    )
...
def check_lines(lines: cpy.LineReturn, line_type: LineType | str) -> None:
    ...
    if line_type == LineType.Separate:
        ...
        for i, (points, codes) in enumerate(zip(*lines)):
            ...
            if len(points) != len(codes):
                raise ValueError(
                    f"Points and codes have different lengths in line {i}"
                )
    elif line_type == LineType.ChunkCombinedCode:
        if TYPE_CHECKING:
            lines = cast(cpy.LineReturn_ChunkCombinedCode, lines)
        _check_tuple_of_lists_with_same_length(
            lines, 2, allow_empty_lists=False
        )
        ...
        _check_tuple_of_lists_with_same_length(
            lines, 1, allow_empty_lists=False
        )
