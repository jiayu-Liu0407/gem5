...
def _convert_filled_from_OuterCode(
    ...
    if fill_type_to == FillType.OuterCode:
        ...
        return (
            filled[0],
            [arr.offsets_from_codes(codes) for codes in filled[1]],
        )
    ...
    if fill_type_to == FillType.ChunkCombinedCode:
        ...
        outer_offsets = (
            None if points is None else arr.offsets_from_lengths(filled[0])
        )
        ...
        if codes is None:
            ret2: cpy.FillReturn_ChunkCombinedOffsetOffset = (
                [None],
                [None],
                [None],
            )
...
def _convert_filled_from_OuterOffset(
    ...
    if fill_type_to == FillType.OuterCode:
        separate_codes = [
            arr.codes_from_offsets(offsets) for offsets in filled[1]
        ]
    ...
    if fill_type_to == FillType.ChunkCombinedCode:
        ...
        for points, offsets in zip(*filled):
            if points is None:
                ...
                chunk_codes.append(
                    arr.codes_from_offsets_and_points(offsets, points)
                )
        ...
        for points, codes, outer_offsets in zip(*filled):
            if points is not None:
                ...
                separate_points += arr.split_points_by_offsets(
                    points, outer_offsets
                )
                separate_codes += arr.split_codes_by_offsets(
                    codes, outer_offsets
                )
...
def convert_filled(
    ...
    if fill_type_from == FillType.OuterCode:
        ...
        return _convert_filled_from_ChunkCombinedCodeOffset(
            filled, fill_type_to
        )
    elif fill_type_from == FillType.ChunkCombinedOffsetOffset:
        if TYPE_CHECKING:
            filled = cast(cpy.FillReturn_ChunkCombinedOffsetOffset, filled)
        return _convert_filled_from_ChunkCombinedOffsetOffset(
            filled, fill_type_to
        )
    else:
...
def _convert_lines_from_Separate(
    ...
    if line_type_to == LineType.Separate:
        ...
        if not lines:
            ...
            ret2 = (
                [arr.concat_points(lines)],
                [arr.offsets_from_lengths(lines)],
            )
        return ret2
...
def _convert_lines_from_SeparateCode(
    ...
    if line_type_to == LineType.Separate:
        ...
        if not lines[0]:
            ...
            ret2 = (
                [arr.concat_points(lines[0])],
                [arr.offsets_from_lengths(lines[0])],
            )
        return ret2
    elif line_type_to == LineType.ChunkCombinedNan:
        if not lines[0]:
...
def _convert_lines_from_ChunkCombinedCode(
    ...
    if line_type_to in (LineType.Separate, LineType.SeparateCode):
        ...
        if line_type_to == LineType.Separate:
            ...
            separate_codes = [
                arr.codes_from_points(line) for line in separate_lines
            ]
...
def convert_multi_lines(
    ...
    return [
        convert_lines(lines, line_type_from, line_type_to)
