...
def concat_codes_or_none(
    ...
    list_of_codes = [
        codes for codes in list_of_codes_or_none if codes is not None
    ]
    if list_of_codes:
...
def concat_offsets(list_of_offsets: list[cpy.OffsetArray]) -> cpy.OffsetArray:
    ...
    ret: cpy.OffsetArray = np.concatenate(
        (
            list_of_offsets[0],
            *(
                list_of_offsets[i + 1][1:] + cumulative[i]
                for i in range(n - 1)
            ),
        ),
        dtype=offset_dtype,
...
def concat_points_or_none(
    ...
    list_of_points = [
        points for points in list_of_points_or_none if points is not None
    ]
    if list_of_points:
...
def concat_points_with_nan(
    list_of_points: list[cpy.PointArray],
) -> cpy.PointArray:
...
def insert_nan_at_offsets(
    ...
    if len(offsets) <= 2:
        ...
        return np.insert(
            points, offsets[1:-1].astype(np.int64), nan_spacer, axis=0
        )
...
def offsets_from_codes(codes: cpy.CodeArray) -> cpy.OffsetArray:
    ...
    return np.append(np.nonzero(codes == MOVETO)[0], len(codes)).astype(
        offset_dtype
    )
...
def offsets_from_lengths(
    ...
    return np.cumsum(
        [0] + [len(line) for line in list_of_points], dtype=offset_dtype
    )
...
def outer_offsets_from_list_of_codes(
    ...
    if not list_of_codes:
        raise ValueError(
            "Empty list passed to outer_offsets_from_list_of_codes"
        )
...
def outer_offsets_from_list_of_offsets(
    ...
    if not list_of_offsets:
        raise ValueError(
            "Empty list passed to outer_offsets_from_list_of_offsets"
        )

    return np.cumsum(
        [0] + [len(offsets) - 1 for offsets in list_of_offsets],
        dtype=offset_dtype,
    )
...
def remove_nan(
    points: cpy.PointArray,
) -> tuple[cpy.PointArray, cpy.OffsetArray]:
    ...
    if len(nan_offsets) == 0:
        ...
        offsets: cpy.OffsetArray = np.empty(
            len(nan_offsets) + 2, dtype=offset_dtype
        )
        offsets[0] = 0
