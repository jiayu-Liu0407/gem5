...
def contour_generator(
    ...
    *,
    ...
    if x.ndim == 0:
        ...
        raise TypeError(
            f"Inputs x and y must be None, 1D or 2D, not {x.ndim}D"
        )
    ...
    if corner_mask is None:
        ...
        raise ValueError(
            f"{name} contour generator does not support corner_mask=True"
        )
    ...
    if quad_as_tri and not cls.supports_quad_as_tri():
        raise ValueError(
            f"{name} contour generator does not support quad_as_tri=True"
        )
